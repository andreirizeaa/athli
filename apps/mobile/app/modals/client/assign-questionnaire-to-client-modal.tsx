...
import { getQuestionnaires, type Questionnaire } from '@/services/coach/coach-questionnaire-service';
import { assignClientQuestionnaire } from '@/services/client/client-form-service';
import { haptics } from '@/utils/haptics';
import { Dialog } from '@/components/ui/dialog';

export default function AssignQuestionnaireToClientModal() {
    ...
    const filteredQuestionnaires = useMemo(() => {
        ...
        return questionnaires.filter((questionnaire) =>
            fuzzyMatch(questionnaire.name.toLowerCase(), query) ||
            (questionnaire.description && fuzzyMatch(questionnaire.description.toLowerCase(), query))
        );
    ...
    const handleSave = useCallback(async () => {
        if (selectedQuestionnaireIds.size === 0 || !clientId || !coachId) return;
        ...
        try {
            ...
            haptics.success();
            await refreshSection('questionnaires');
            handleClose();
        } catch (error) {
            haptics.error();
    ...
    }, [handleClose, selectedQuestionnaireIds, clientId, coachId, refreshSection]);

    const canSave = selectedQuestionnaireIds.size > 0 && !isSaving;

    const handleQuestionnaireToggle = useCallback((questionnaireId: string) => {
            ...
            <View style={[styles.fixedHeader, { height: headerHeight }]}>
                ...
                >
                    ...
                    <Text style={[styles.title, { color: themeColors.text }]}>
                        {t('clientDetail.assignModals.assignQuestionnaire')}
                    </Text>
                    <IconButton
            ...
            <View style={styles.content}>
                <FlashList
                    ...
                    renderItem={({ item, index }) => {
                        const isSelected = selectedQuestionnaireIds.has(item.id);
                        ...
                        return (
                            <View>
                                <PressableOpacity
                                    onPress={() => handleQuestionnaireToggle(item.id)}
                    ...
                    ListEmptyComponent={
                        <EmptyState message={t('library.empty.questionnaires')} />
                    }
