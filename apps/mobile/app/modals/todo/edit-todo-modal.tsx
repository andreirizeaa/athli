...
export default function EditTodoModal() {
  ...
  const { ownTodos, updateOwnTodo, isUpdating } = useCoachOwnTodos();
  ...
  const {
    ...
    initialClientAvatar?: string;
  }>();

  // Form state - initialize from params
  const [type, setType] = useState<TodoType>((initialType as TodoType) || 'general');
  const [selectedClient, setSelectedClient] = useState<Client | null>(
    initialClientId
      ? {
          id: initialClientId,
          name: initialClientName || '',
          avatarUrl: initialClientAvatar,
          firstName: '',
          lastName: '',
        } as Client
      : null
  ...
  useEffect(() => {
    if (!initialStateRef.current) {
      // Normalize the date through Date object to match how we compare later
      const normalizedDueDate = initialDueDate ? new Date(initialDueDate).toISOString() : '';
      initialStateRef.current = {
        type: (initialType as TodoType) || 'general',
  ...
  useEffect(() => {
    if (initialStateRef.current) {
      ...
      const dirty = !areStatesEqual(currentState, initialStateRef.current);
      setIsDirty(dirty);
    }
  }, [type, selectedClient, title, description, selectedDate]);
  ...
  const canComplete = useMemo(() => {
    ...
    return formValid && clientValid;
  }, [title, selectedClient, type]);
  ...
  const handleSave = useCallback(async () => {
    if (!canComplete || isUpdating || !todoId) return;
  ...
  }, [canComplete, isUpdating, todoId, updateOwnTodo, title, description, type, selectedClient, selectedDate, router]);
      ...
      <View style={[styles.fixedHeader, { height: headerHeight }]}>
        ...
        >
          ...
          <Text style={[styles.title, { color: themeColors.text }]}>
            {t('todos.editTask.title')}
          </Text>
          <IconButton
            icon={{ sf: 'checkmark', IconComponent: Check }}
            onPress={handleSave}
            size="md"
            variant={canComplete && isDirty && !isUpdating ? 'primary' : 'default'}
            disabled={!canComplete || !isDirty || isUpdating}
            loading={isUpdating}
          />
        </View>
      </View>
