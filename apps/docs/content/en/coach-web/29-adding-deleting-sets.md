...
**What happens if I link sets after entering different values?**
Linking sets after entering different values will synchronise all sets to match the first set's values. Make sure you intend to overwrite before linking.

**Can I add sets to exercises inside a superset?**
Yes. Each exercise within a superset, triset, or giant set can have its own number of sets managed independently.

**Is there a maximum number of sets per exercise?**
There is no strict maximum, but the interface is optimised for typical training volumes. Extremely high set counts remain functional but may require scrolling.

**Do added sets sync to the client's app immediately?**
Changes are saved when you save the workout. Once saved, the updated sets will be visible to the client the next time they open the workout in the mobile app.
