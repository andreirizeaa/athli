...
export function DataGrid<T extends Record<string, any>>({
  ...
  const setSelectedRowIds: (ids: Set<string> | ((prev: Set<string>) => Set<string>)) => void = onSelectionChange
  ...
  useEffect(() => {
    ...
    const filtersKey = JSON.stringify(filters.map(f => ({ id: f.id, defaultValue: f.defaultValue })))
  ...
  const filteredData = useMemo(() => {
    ...
    filters.forEach((filter) => {
      ...
      if (filterValue) {
        result = result.filter((row) => {
          const rowValue = filter.getFilterValue ? filter.getFilterValue(row) : (row[filter.id] as string)
  ...
  useEffect(() => {
    if (selectedRowIds.size > 0 && tableBodyRef.current) {
      ...
      const selectedRow = tableBodyRef.current.querySelector(`[data-row-id="${selectedId}"]`) as HTMLTableRowElement
      if (selectedRow) {
  ...
  const isAllSelected = paginatedData.length > 0 && paginatedData.every((row) => selectedRowIds.has(getRowId(row)))
  const isIndeterminate = paginatedData.some((row) => selectedRowIds.has(getRowId(row))) && !isAllSelected
  ...
  return (
    <div className={cn("h-full w-full flex flex-col min-h-0", compactMode && "-mx-4 w-[calc(100%+2rem)]")}>
      ...
      <div className="w-full flex-1 flex flex-col overflow-hidden">
              ...
              return (
                <DropdownMenu key={filter.id}>
                  ...
                  <DropdownMenuContent align="end" className="w-56">
                    ...
                    >
                      <DropdownMenuRadioItem value="all" className={cn(filterValues[filter.id] === null && "bg-accent")}>
                      ...
                      {filter.options.map((option) => (
                        ...
                        >
                          ...
                          {filterValues[filter.id] === option.value && <Check className="ml-2 size-4" />}
                        </DropdownMenuRadioItem>
                      ))}
                    </DropdownMenuRadioGroup>
                  </DropdownMenuContent>
        ...
        >
          ...
          <table className="table-fixed border-separate border-spacing-0 w-full" style={{ tableLayout: "fixed", width: "100%" }}>
            <colgroup>
              {stickyFirstColumn && (
                <col style={{ width: firstColumnWidth, minWidth: firstColumnWidth, maxWidth: firstColumnWidth }} />
              )}
              ...
              <TableRow className="hover:bg-transparent h-10">
                      {stickyFirstColumn && renderFirstColumn && (
                        ...
                        >
                          {renderFirstColumnHeader ? (
                            renderFirstColumnHeader({
                              isSorted: sortColumn === (firstColumnId || columns.find((col) => col.id === filteredColumnOrder[0])?.id || null),
                              isAscending: sortColumn === (firstColumnId || columns.find((col) => col.id === filteredColumnOrder[0])?.id || null) && sortDirection === "asc",
                              isDescending: sortColumn === (firstColumnId || columns.find((col) => col.id === filteredColumnOrder[0])?.id || null) && sortDirection === "desc",
                              onSort: (direction) => {
                                const columnIdToSort = firstColumnId || columns.find((col) => col.id === filteredColumnOrder[0])?.id
                                if (columnIdToSort) {
          ...
          >
            <div className="flex items-center gap-1">
              ...
              >
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      "px-2 gap-1.5",
                      compactMode ? "h-7 text-xs" : "h-9"
                    )}
                  >
                    <span className={cn(compactMode && "text-xs")}>{currentPage}</span>
                    <ChevronDown className={cn("text-foreground", compactMode ? "size-3" : "size-4")} />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent
                  align="center"
                  ...
                  style={pageDropdownWidth ? { width: `${pageDropdownWidth}px`, minWidth: `${pageDropdownWidth}px` } : undefined}
                >
                  <DropdownMenuRadioGroup
                  ...
                  >
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                      ...
                      >
                        ...
                        {currentPage === page && <Check className={cn("size-4", compactMode && "size-3")} />}
                      </DropdownMenuRadioItem>
                    ))}
                  </DropdownMenuRadioGroup>
                </DropdownMenuContent>
              ...
              <Button
                ...
                className={cn(compactMode && "h-7 w-7")}
              >
                <ChevronRight className={cn("text-foreground", compactMode ? "size-3" : "size-4")} />
              </Button>
            </div>
