...
const QuestionnaireDetailPage = () => {
  ...
  const [questionnaire, setQuestionnaire] = useState<ClientQuestionnaireDetail | null>(null);
  ...
  useEffect(() => {
    const fetchQuestionnaire = async () => {
      if (!clientId || !questionnaireId) return;

      setIsLoading(true);
      try {
        const data = await getClientQuestionnaire(clientId, questionnaireId);
  ...
  const handleDownload = async () => {
    if (!questionnaire || isDownloading) return;
  ...
  const getAnswerForQuestion = (questionId: string): QuestionAnswer | undefined => {
    return questionnaire?.answers.find((a) => a.questionId === questionId);
  };
    ...
    return (
      <div className="h-full w-full flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-semibold mb-2">Questionnaire not found</h1>
          <p className="text-muted-foreground">The questionnaire you're looking for doesn't exist.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full flex flex-col bg-background">
      <div className="w-full relative flex-shrink-0">
        <div className="px-4 flex items-start justify-between gap-4 mb-2 mt-2">
          <div className="flex flex-col gap-1 flex-1 min-w-0">
            ...
            <h1 className="text-[22px] font-semibold">{questionnaire.name}</h1>
            {questionnaire.description && (
              <p className="text-sm text-muted-foreground -mt-[6px]">{questionnaire.description}</p>
            )}
          </div>
          <Button
            onClick={handleDownload}
            disabled={isDownloading}
            className="flex-shrink-0 gap-2 min-w-[120px]"
          >
            {isDownloading ? (
      ...
      <div className="w-full px-4 py-4 flex items-start justify-center">
        <Card className="w-full max-w-2xl">
          <div className="w-full">
            {questionnaire.completedAt && (
              <>
                <CardHeader className="px-4 pb-3">
                  <CardTitle>{formatCompletionDate(questionnaire.completedAt)}</CardTitle>
                </CardHeader>
            ...
            {questionnaire.questions.map((question, index) => (
              <div
                ...
                className={cn(
                  'py-4 px-4',
                  index < questionnaire.questions.length - 1 && 'border-b'
                )}
              >
