      ...
      {viewMode === 'list' ? (
        <div className="flex flex-col gap-2 mt-4 overflow-y-auto flex-1 min-h-0">
          {exerciseResults.slice(0, 40).map((exercise) => {
            const isActive = activeExerciseIds.has(exercise.exerciseId);
            return (
            <Card
              ...
              tabIndex={0}
              className={cn(
                "cursor-grab active:cursor-grabbing transition-colors overflow-hidden min-h-[60px] h-[60px] rounded-md shadow-none group/card",
        ...
        <div className="grid gap-3 mt-4 overflow-y-auto flex-1 min-h-0 items-start" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', alignContent: 'start' }}>
          {exerciseResults.slice(0, 40).map((exercise) => {
            const isActive = activeExerciseIds.has(exercise.exerciseId);
            return (
            <Card
              ...
              tabIndex={0}
              className={cn(
                "cursor-grab active:cursor-grabbing overflow-hidden rounded-md shadow-none flex flex-col group/card p-0 gap-0 h-[135px] min-w-[100px]",
                isActive && "border-primary border-2 bg-primary/5"
              )}
              aria-label={`Select ${exercise.name} exercise`}
            >
