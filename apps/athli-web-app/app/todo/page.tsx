...
const TodoPage = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'outstanding' | 'completed' | 'overdue'>('outstanding');
  const [taskTypeFilter, setTaskTypeFilter] = useState<'all' | 'client' | 'general'>('all');
  ...
  const filteredAndSortedTasks = useMemo(() => {
    const filtered = tasks.filter((task) => {
      ...
      const matchesStatus =
        statusFilter === 'all' ||
        (statusFilter === 'outstanding' && !task.completed) ||
        (statusFilter === 'completed' && task.completed) ||
        (statusFilter === 'overdue' && isOverdue);

      const matchesTaskType =
        taskTypeFilter === 'all' ||
        (taskTypeFilter === 'client' && task.type === 'client') ||
        (taskTypeFilter === 'general' && task.type === 'general');
    ...
    return sorted;
  }, [tasks, searchQuery, statusFilter, taskTypeFilter, sortOrder]);
  ...
  return (
    <div className="h-full w-full flex flex-col">
      ...
      <div className="w-full flex-1 min-h-0 overflow-hidden px-4 py-4">
        <div className="h-full w-full flex flex-col bg-background border rounded-lg">
          <div className="px-4 py-3 border-b flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2">
              ...
              <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as 'all' | 'outstanding' | 'completed' | 'overdue')}>
              ...
              <Select value={taskTypeFilter} onValueChange={(value) => setTaskTypeFilter(value as 'all' | 'client' | 'general')}>
