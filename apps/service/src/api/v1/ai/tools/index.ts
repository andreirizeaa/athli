...
export const createGetClientWorkoutsTool = (ctx: ToolContext) =>
  tool(
    async ({ clientId, limit = 20 }: { clientId: string; limit?: number }) => {
      const supabase = getSupabaseClient();
...
export const createGetClientCheckinsTool = (ctx: ToolContext) =>
  tool(
    async ({ clientId, limit = 10 }: { clientId: string; limit?: number }) => {
      const supabase = getSupabaseClient();
...
export const createSearchExercisesTool = (ctx: ToolContext) =>
  tool(
    async ({ query, muscleGroup, equipment, limit = 20 }: { query?: string; muscleGroup?: string; equipment?: string; limit?: number }) => {
      const supabase = getSupabaseClient();
...
export const createGetCoachWorkoutsTool = (ctx: ToolContext) =>
  tool(
    async ({ limit = 50, filter }: { limit?: number; filter?: string }) => {
      const supabase = getSupabaseClient();
...
export const createGetExerciseCatalogTool = (ctx: ToolContext) =>
  tool(
    async ({ muscle, category, difficulty, force, mechanic }: { muscle?: string; category?: string; difficulty?: string; force?: string; mechanic?: string }) => {
      const supabase = getSupabaseClient();
...
export const createAssignWorkoutTool = (ctx: ToolContext) =>
  tool(
    async (input: AssignWorkoutInput) => {
...
export const createAssignMetricToClientTool = (ctx: ToolContext) =>
  tool(
    async ({ clientId, clientName, metricId, metricName }: { clientId: string; clientName?: string; metricId: string; metricName?: string }) => {
...
export const createAddClientGoalTool = (ctx: ToolContext) =>
  tool(
    async ({ clientId, clientName, goalType, targetValue, targetDate, description }: { clientId: string; clientName?: string; goalType: string; targetValue?: string; targetDate?: string; description?: string }) => {
...
export const createAddClientInjuryTool = (ctx: ToolContext) =>
  tool(
    async ({ clientId, clientName, injuryType, bodyPart, severity, notes, dateOccurred }: { clientId: string; clientName?: string; injuryType: string; bodyPart: string; severity?: string; notes?: string; dateOccurred?: string }) => {
...
export const createDraftMessageTool = (ctx: ToolContext) =>
  tool(
    async ({ clientId, clientName, message, messageType }: { clientId: string; clientName?: string; message: string; messageType?: string }) => {
      return JSON.stringify({
        success: true,
...
export const createUpdateClientProfileTool = (ctx: ToolContext) =>
  tool(
    async ({ clientId, clientName, updates }: { clientId: string; clientName?: string; updates: Record<string, unknown> }) => {
