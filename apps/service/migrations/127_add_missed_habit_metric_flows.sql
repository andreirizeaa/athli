-- ================================================
-- ATHLI Add Missed Habit Log & Missed Metric Log Flows
-- ================================================

-- STEP 1: Update handle_new_coach_setup function to seed 6 default flows (was 4)
...
BEGIN
  ...
  IF NEW.user_type = 'coach' THEN
    ...
    -- 4. Create default flows (6 fixed flows)
...
-- STEP 3: Backfill existing coaches with the 2 new flows
DO $$
DECLARE
    coach RECORD;
BEGIN
    FOR coach IN SELECT id FROM public.user_profiles WHERE user_type = 'coach' LOOP
        ...
        WHERE NOT EXISTS (
            ...
            WHERE coach_id = coach.id AND lower(name) = lower('Missed Habit Log')
        );
        ...
        WHERE NOT EXISTS (
            SELECT 1 FROM public.coach_flows
            WHERE coach_id = coach.id AND lower(name) = lower('Missed Metric Log')
        );
