-- ================================================
-- 096: Add Realtime Broadcast Triggers
--
-- Creates triggers to broadcast message changes via Supabase Realtime.
-- Uses broadcast events instead of postgres_changes for better scalability.
-- ================================================

-- Broadcast trigger for messages table
...
BEGIN
  ...
  -- Parameter order: payload, event, topic, is_private
...
GRANT EXECUTE ON FUNCTION public.broadcast_message_changes() TO authenticated;

-- Attach trigger to messages table
DROP TRIGGER IF EXISTS trg_broadcast_message_changes ON public.messages;
CREATE TRIGGER trg_broadcast_message_changes
...
COMMENT ON FUNCTION public.broadcast_message_changes() IS
  'Broadcasts message changes to Supabase Realtime channel for real-time messaging';
