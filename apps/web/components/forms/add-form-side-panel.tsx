...
type AddFormSidePanelProps = {
  ...
  editingForm?: (CheckIn | Questionnaire) | null;
};
...
export const AddFormSidePanel = ({ open, onOpenChange, onSave, editingForm }: AddFormSidePanelProps) => {
  ...
  const handleSave = async (values: FormFormValues) => {
    ...
    try {
      if (isEditing && editingForm) {
        const isCheckIn = editingForm.id.startsWith('checkin-');
  ...
  const hasChanges = isEditing && editingForm && (
    form.watch('name') !== editingForm.name ||
    form.watch('description') !== (editingForm.description || '')
  );

  const isValid = isEditing
    ? form.formState.isValid && form.watch('name').trim() !== '' && !!hasChanges
    : activeTab === 'new'
