...
import { SectionType } from '@/app/(app)/library/training/sections/section-type-utils';
...
function WorkoutBuilderScreenshotPreview({ aiAnimationData }: { aiAnimationData: object | null }) {
    ...
    <div className="flex flex-col items-center gap-4">
      ...
      <div ref={containerRef} className="relative w-full">
        {w > 0 && h > 0 && (
          ...
          >
            <defs>
              <linearGradient id="border-grad-workout-builder" x1="0.5" y1="0" x2="0.5" y2="1">
            ...
            <motion.rect
              ...
              ry={r}
              pathLength={1}
              stroke="url(#border-grad-workout-builder)"
              strokeWidth={3}
              strokeLinecap="round"
...
export const WorkoutBuilder = ({
  ...
  //      * Sections (itemType: 'section') with types: 'regular', 'amrap', 'tabata', 'hiit', 'emom', 'auxiliary'
  ...
  //    - Sections need: id, name, type, notes, and type-specific fields (durationSec for amrap, workSec/restSec/rounds for tabata/hiit, intervalSec/durationMin for emom)
  //
  // 4. VALIDATION:
...
Focus on proper form and progressive overload.`;
  ...
  const handleSave = async () => {
    ...
    // Use updated values from state - create meta even if original meta is null (for new workouts via AI)
    const updatedMeta = {
      ...(meta || {}),
      name: workoutTitle,
      type: workoutType || '',
  ...
  const handleSaveSectionToLibrary = async (section: WorkoutSection) => {
    ...
    try {
      ...
      toast.success(t('toasts.sectionSavedToLibrary'));
    } catch (error) {
      console.error('Error saving section to library:', error);
      toast.error(t('toasts.failedSaveSectionToLibrary'));
    } finally {
    ...
    <>
                            ...
                            {(() => {
                              ...
                              while (i < workoutSchema.items.length) {
                                ...
                                if (startsSuperset && item.exercise) {
                                  ...
                                  // Check for drop zones and spacing
                                  const isPrevSection = firstItemPrev?.itemType === 'section';
                                  const isNextSection = lastItemNext?.itemType === 'section';
                                  ...
                                  const showDropZoneBefore = isDraggingSomething && dragOverTopLevelSlot === i && canShowDropZoneBefore;
                                  const showDropZoneAfter = isDraggingSomething && dragOverTopLevelSlot === j && canShowDropZoneAfter;
      ...
      <Dialog open={isUpgradeDialogOpen} onOpenChange={setIsUpgradeDialogOpen}>
        <DialogContent className="sm:max-w-2xl">
          ...
          <div className="py-4">
            <WorkoutBuilderScreenshotPreview aiAnimationData={aiAnimationData} />
          </div>
          <DialogFooter>
