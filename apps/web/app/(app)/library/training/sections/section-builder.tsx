...
export const SectionBuilder = ({
  ...
  // Initialize with a single section
  const getInitialSchema = (): WorkoutSchema => {
    ...
    if (initialData) {
      ...
      const converted = convertPayloadToBuilderFormat(initialData, findExerciseById);
      return converted;
    }

    // Default: create initial section
    return {
      items: [{
  ...
  // Ensure section name stays in sync with workout title in section mode
  useEffect(() => {
    setWorkoutSchema((prev) => {
      const firstItem = prev.items[0];
      if (
        firstItem &&
        firstItem.itemType === 'section' &&
        firstItem.section.name !== workoutTitle
      ) {
        return {
          ...prev,
          items: prev.items.map((item, idx) => {
            if (idx === 0 && item.itemType === 'section') {
              return {
                ...item,
                section: { ...item.section, name: workoutTitle },
              };
            }
            return item;
          }),
                        ...
                        <div className="flex items-center gap-2 w-full">
                          ...
                          <Popover open={isTitleExpanded} onOpenChange={setIsTitleExpanded}>
                            <Tooltip>
                              ...
                              <TooltipContent>
                                {t('workouts.addWorkout.openSectionDetails')}
                              </TooltipContent>
                            </Tooltip>
                            <PopoverContent className="w-[600px] p-4" align="end">
                              <div className="flex flex-col gap-4">
                                {/* Type - hidden in section mode */}
                                <div className="flex flex-col gap-2">
                                </div>

                                {/* Difficulty - hidden in section mode */}
                                <div className="flex flex-col gap-2">
                                </div>
