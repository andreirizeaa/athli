export const enMessages = {
  layout: {
    appTitle: 'Athli',
    appDescription: 'Generated by create next app',
  },
  getStarted: {
    welcomeTitle: 'Welcome to Athli!',
    welcomeSubtitle:
      'Explore our powerful tools to help you save time and grow your training business.',
    cards: {
      clientApp: {
        title: 'Experience Client App',
        description:
          'See exactly what your clients experience when using the Athli mobile app through a full demo prepared for you.',
        button: 'Launch Demo',
      },
      features: {
        title: "Explore Athli's Powerful Features",
        description:
          'Discover all the features that make Athli the ultimate coaching platform. From workout builders to habit tracking, explore everything you can do.',
        button: 'Explore Features',
      },
      coachApp: {
        title: 'Experience the Coach App',
        description:
          "Experience Athli's lightweight mobile app designed specifically for coaches. Message clients, manage workouts, assign items, and stay on top of your training business on the go.",
        button: 'Start Tour',
      },
    },
    checklist: {
      workouts: 'Workouts',
      aiAssistant: 'AI Assistant',
      flows: 'Flows',
      habits: 'Habits',
      metrics: 'Metrics',
      forms: 'Forms',
      automations: 'Automations',
      explore: 'Explore',
      details: {
        workouts: {
          title: 'Build workouts with AI',
          subtitle: 'Create personalized workouts in seconds using our intelligent workout builder.',
        },
        aiAssistant: {
          title: 'Your AI-powered coaching assistant',
          subtitle: 'Get intelligent suggestions, automate tasks, and enhance your coaching workflow.',
        },
        flows: {
          title: 'Build powerful automations',
          subtitle: 'Create workflows that trigger actions based on client activity.',
        },
        habits: {
          title: 'Track lifestyle habits',
          subtitle: 'Help clients build healthy habits with daily tracking and streaks.',
        },
        metrics: {
          title: 'Track any metric',
          subtitle: 'Monitor weight, body measurements, or any custom metric over time.',
        },
        forms: {
          title: 'Create check-ins & forms',
          subtitle: 'Build questionnaires to gather feedback, progress updates, and client data.',
        },
        automations: {
          title: 'Automate client onboarding',
          subtitle: 'Set up automated flows to welcome new clients and collect information.',
        },
      },
    },
    explored: '{count}/{total} explored',
    productGuides: 'Product Guides',
    demoVideo: {
      title: 'Demo video',
      description: 'See how Athli will empower you.',
    },
    helpArticles: {
      title: 'Help Articles',
      description:
        'Comprehensive articles and step-by-step instructions designed to help you use Athli with clarity and confidence.',
      button: 'Discover',
    },
    liveChat: {
      title: 'Live Chat Support',
      description:
        "Questions, concerns, bugs, feature requests, or feedback? Our team is here to help you succeed. Chat with us in real-time.",
    },
    referAndEarn: {
      title: 'Share Athli, get $20',
      description:
        'Invite your coaching friends to Athli. When they subscribe, you both get $20 credit.',
      giftWaiting: 'A gift is waiting',
    },
  },
  home: {
    authenticatedContent: 'Authenticated content',
    todayIs: 'Today is the {day} of {month}, {year}',
    programmingRequired: 'Programming required',
    toDoList: 'To do list',
    messages: 'Messages',
    addTask: 'Add task',
    taskTitle: 'Title',
    taskTitlePlaceholder: 'Enter task title...',
    taskInformation: 'Information',
    taskInformationPlaceholder: 'Enter task information...',
    taskType: 'Task type',
    taskTypeClient: 'Client',
    taskTypeGeneral: 'General',
    selectClient: 'Select client',
    selectClientPlaceholder: 'Select a client...',
    completeBy: 'Complete by',
    selectDate: 'Select date',
    taskTitleRequiredError: 'Task title is required',
    completeByRequiredError: 'Complete by date is required',
    clientIdRequiredError: 'Client selection is required',
    task: 'Task',
    due: 'Due',
    type: 'Type',
    action: 'Action',
    actions: 'Actions',
    athlete: 'Athlete',
    workout: 'Workout',
    filterAll: 'All',
    filterOutstanding: 'Outstanding',
    filterCompleted: 'Completed',
    filterOverdue: 'Overdue',
    filterRequired: 'Required',
    filterUpcoming: 'Upcoming',
    filterCompletedClient: 'Completed Client',
    filterOutstandingClient: 'Outstanding Client',
    filterCompletedGeneral: 'Complete General',
    filterOutstandingGeneral: 'Outstanding General',
    sortEarliest: 'Sort earliest',
    sortLatest: 'Sort latest',
    noTasksFound: 'No tasks found',
    noUnreadMessages: 'No unread messages',
    client: 'Client',
    unread: 'Unread',
    noProgrammingRequired: 'No programming required',
    lastScheduledWorkout: 'Last scheduled workout is on {date}',
    trainingCalendar: 'Training calendar',
    viewTrainingCalendar: 'View training calendar',
    viewMessages: 'View messages',
    completedWorkouts: 'Completed workouts',
    missedWorkouts: 'Missed Workouts',
    switchToMissedWorkouts: 'Switch to Missed Workouts',
    switchToCompletedWorkouts: 'Switch to Completed Workouts',
    today: 'Today',
    yesterday: 'Yesterday',
    noMissedWorkouts: 'No missed workouts',
    noCompletedWorkouts: 'No completed workouts',
    exercises: 'Exercises',
    minutes: 'Minutes',
    intensity: 'Intensity',
    weight: 'Weight',
    viewProfile: 'View profile',
    viewSessionComments: 'View session comments',
    noSessionComments: 'No session comments',
    viewWorkout: 'View workout',
    editWorkout: 'Edit workout',
    messageClient: 'Message client',
    seeProfile: 'See profile',
    selectDateRange: 'Select date range',
    fromDate: 'From',
    toDate: 'To',
    markComplete: 'Mark as complete',
    markIncomplete: 'Mark as incomplete',
    editTask: 'Edit task',
    yourList: 'Your list',
    yourListEmptyStateTitle: 'No tasks in your list',
    athliAssistant: 'Athli assistant',
    athliAssistantEmptyStateTitle: 'Athli Assistant is resting',
    summaryCards: {
      todos: 'To do',
      checkIns: 'Check-ins',
      outstanding: 'Outstanding',
      awaitingReview: 'Awaiting review',
      allCaughtUp: 'All caught up!',
    },
    atRiskClients: {
      title: 'At Risk',
      noActivity: 'No activity',
      lastActivityOne: 'Last activity 1 day ago',
      lastActivityOther: 'Last activity {count} days ago',
      infoTooltip: 'What does at risk mean?',
      infoDialogTitle: 'At Risk Clients',
      infoDialogMessage: 'Clients shown here have not logged any training activity in the last 5 days. Regular check-ins can help keep them engaged and on track with their fitness goals.',
    },
  },
  dashboard: {
    title: 'Dashboard',
    comingSoon: 'Coming Soon',
  },
  features: {
    manageClients: 'Manage all your clients',
    assignSchedule: 'Assign schedule',
    customExercises: 'Custom exercises',
    messageAthletes: 'Message athletes directly',
  },
  settings: {
    title: 'Settings',
    searchPlaceholder: 'Search settings...',
    discardChanges: {
      title: 'Discard changes?',
      description: 'You have unsaved changes. Are you sure you want to leave? Your changes will be lost.',
      cancel: 'Cancel',
      confirm: 'Discard',
    },
    groups: {
      personal: 'Personal Settings',
      billing: 'Billing',
      appSettings: 'App Settings',
      business: 'Business',
    },
    sections: {
      account: 'Account',
      general: 'General',
      profile: 'Profile',
      security: 'Security',
      notifications: 'Notifications',
      customisations: 'Customisations',
      integrations: 'Integrations',
      email: 'Email',
      company: 'Company',
      billing: 'Billing',
      billingCurrent: 'Current',
      billingPlans: 'Plans',
      companyInformation: 'Information',
      companyBranding: 'Branding',
      companyTeam: 'Team',
      accountProfile: 'Profile',
      accountSecurity: 'Security',
      accountInformation: 'Information',
      accountDanger: 'Danger',
    },
    integrations: {
      connected: {
        title: 'Connected integrations',
      },
      available: {
        title: 'Available',
      },
      comingSoon: {
        title: 'Coming Soon',
        comingSoon: 'Coming Soon',
      },
      application: 'APPLICATION',
      action: 'ACTION',
      connect: 'Connect',
      disconnect: 'Disconnect',
      noIntegrationsAvailable: 'No integrations available',
      disconnectedSuccessfully: 'Integration disconnected successfully',
      disconnectFailed: 'Failed to disconnect integration',
      calendar: {
        title: 'Calendar',
        description: 'Configure your calendar integration settings here.',
        connectCalendar: 'Connect Calendar',
        connectYourCalendar: 'Connect Your Calendar',
        connectDescription: 'Connect your calendar to manage appointments directly from Athli.',
        connected: 'Calendar Connected',
        connectedDescription: 'Your calendar is connected and ready to use.',
        gmail: 'Gmail',
        outlook: 'Outlook',
        checkingConnection: 'Checking calendar connection...',
        failedToCheckStatus: 'Failed to check calendar connection status',
        connect: {
          title: 'Connect Calendar',
          connectProvider: 'Connect {provider}',
          onlyOneCalendar: 'Only one calendar per user can be connected.',
          description: 'Only one Google or Microsoft (365, Outlook) calendar per user can be connected.',
          emailAddress: 'Email address',
          emailPlaceholder: 'your.email@example.com',
          emailAria: 'Email address for calendar connection',
          detectingProvider: 'Detecting calendar provider...',
          unableToDetect: 'Unable to detect calendar provider. Please ensure your email uses Google Workspace or Microsoft 365.',
          providerMismatch: 'The email address you entered is linked to a different provider: {detected}, but you selected {selected}. Please use the corrent providers email address.',
          cancel: 'Cancel',
          cancelAria: 'Cancel calendar connection',
          connectAria: 'Connect calendar',
          connecting: 'Connecting...',
          checking: 'Checking...',
          pleaseEnterEmail: 'Please enter a valid email address',
          unableToDetectProvider: 'Unable to detect calendar provider. Please check your email address.',
          connectionFailed: 'Failed to connect calendar',
        },
        callback: {
          connecting: 'Connecting your calendar...',
          connectionFailed: 'Failed to connect calendar',
          noTokensReturned: 'No tokens returned from OAuth provider',
          calendarConnectionFailed: 'Calendar connection failed. No access token returned.',
          failedToSave: 'Failed to save calendar connection',
        },
        toast: {
          connectedSuccessfully: 'Calendar connected successfully!',
          connectionFailed: 'Failed to connect calendar',
        },
        disconnectedSuccessfully: 'Calendar disconnected successfully',
        disconnectFailed: 'Failed to disconnect calendar',
        onlyOneActive: 'Only one calendar can be active at a time. Please disconnect your current calendar before connecting a new one.',
      },
    },
    customisations: {
      saveFailed: 'Failed to save customisations',
      savedSuccessfully: 'Customisations saved successfully',
      preferences: {
        title: 'Preferences',
        language: {
          label: 'Language',
        },
        theme: {
          label: 'Theme',
        },
        color: {
          label: 'Color Theme',
        },
        terminology: {
          label: 'Terminology',
          athlete: 'Athlete',
          client: 'Client',
          member: 'Member',
        },
      },
    },
    profile: {
      accountInfo: 'Account Info',
      profileInformation: 'Profile Information',
      profilePicture: 'Profile Picture',
      upload: 'Upload',
      uploading: 'Uploading...',
      uploadAria: 'Upload profile picture',
      delete: 'Delete',
      deleteAria: 'Delete profile picture',
      firstName: 'First Name',
      firstNameRequired: 'First Name *',
      firstNamePlaceholder: 'Enter first name',
      firstNameRequiredError: 'First name is required',
      lastName: 'Last Name',
      lastNameRequired: 'Last Name *',
      lastNamePlaceholder: 'Enter last name',
      lastNameRequiredError: 'Last name is required',
      email: 'Email',
      emailPlaceholder: 'Enter email address',
      saveChanges: 'Save Changes',
      saving: 'Saving',
      updatedSuccessfully: 'Profile updated successfully',
      updateFailed: 'Failed to update profile',
      imageUploadFailed: 'Failed to upload image',
      imageFileRequired: 'Please upload an image file',
      imageSizeError: 'Image size must be less than 5MB',
      imageUpdated: 'Profile image updated',
      editEmail: 'Edit',
      editEmailAria: 'Edit email address',
      connectedAccounts: 'Connected accounts',
      connectAccount: 'Connect account',
      connectAccountAria: 'Connect account',
      connectAccountError: 'Failed to connect account. Please try again.',
      noConnectedAccounts: 'No connected accounts',
      profilePictureDialog: {
        title: 'Update Profile Picture',
        description: 'Upload an image for your profile picture',
        clickToUpload: 'Click to upload or drag and drop',
        fileTypes: 'PNG, JPG up to 5MB',
        zoom: 'Zoom',
        rotate: 'Rotate',
        imageFileError: 'Please select an image file',
        imageSizeError: 'Image size must be less than 5MB',
        saveFailed: 'Failed to save profile picture',
      },
      verification: {
        title: 'Additional verification required',
        description: 'Additional verification is required to connect external accounts. Please verify your identity through your account settings, then try connecting again.',
        openProfile: 'Open account settings',
        retry: 'Retry connection',
      },
      changeEmail: {
        title: 'Change email',
        newEmail: 'New email',
        newEmailRequired: 'New email *',
        newEmailPlaceholder: 'Enter new email address',
        confirmEmail: 'Confirm email',
        confirmEmailRequired: 'Confirm email *',
        confirmEmailPlaceholder: 'Confirm new email address',
        changeEmailButton: 'Change email',
        newEmailRequiredError: 'New email is required',
        confirmEmailRequiredError: 'Confirm email is required',
        emailsDoNotMatch: 'Emails do not match',
        invalidEmail: 'Please enter a valid email address',
      },
    },
    security: {
      password: 'Password',
      email: 'Email',
      newPassword: 'New Password',
      confirmPassword: 'Confirm Password',
      emailAddress: 'Email Address',
      passwordRequirements: 'Min 8 chars with uppercase, lowercase, digit & special character',
      showPassword: 'Show password',
      hidePassword: 'Hide password',
      showPasswordAria: 'Show password',
      hidePasswordAria: 'Hide password',
      passwordUpdated: 'Password updated successfully',
      passwordUpdateFailed: 'Failed to update password',
      passwordsDoNotMatch: 'Passwords do not match',
      passwordTooShort: 'Password must be at least 8 characters long',
      passwordMissingLowercase: 'Password must contain at least one lowercase letter',
      passwordMissingUppercase: 'Password must contain at least one uppercase letter',
      passwordMissingDigit: 'Password must contain at least one digit',
      passwordMissingSpecial: 'Password must contain at least one special character',
      emailUpdateSuccess: 'Profile updated. Please check your email to confirm the new address.',
      emailUpdateInitiated: 'Email update initiated. Please check your email to confirm the new address.',
      emailOTPSent: 'Verification code sent to {email}. Please check your email and enter the code.',
      emailUpdateFailed: 'Failed to update email',
      editEmail: 'Edit email',
      currentPassword: 'Current Password',
      currentPasswordRequired: 'Current password is required to change your email',
      currentPasswordIncorrect: 'Current password is incorrect',
      emailChange: {
        verifyPassword: 'Verify Your Password',
        verifyPasswordDescription: 'Please enter your current password to continue.',
        enterNewEmail: 'Enter New Email',
        enterNewEmailDescription: 'Enter the new email address you want to use.',
        newEmail: 'New Email',
        newEmailPlaceholder: 'Enter new email address',
        verifyOTP: 'Verify Your Email',
        verifyOTPDescription: 'We sent a 6-digit verification code to {email}. Please enter it below.',
        verifying: 'Verifying...',
        submit: 'Update Email',
      },
      passwordConfirmation: {
        title: 'Verify Your Password',
        description: 'Please enter your current password to confirm this change.',
        verify: 'Verify',
        verifying: 'Verifying...',
        verifyFailed: 'Password verification failed',
      },
      googleAccount: {
        title: 'Manage your account with Google',
        description: 'Your account is managed through Google. To change your password or email, please visit your Google Account settings.',
        manageAccount: 'Manage Google Account',
      },
      otpVerification: {
        title: 'Verify Your Identity',
        description: 'We sent a 6-digit verification code to {email}. Please enter it below to confirm your changes.',
        enterCode: 'Enter verification code',
        verify: 'Verify',
        verifying: 'Verifying...',
        resend: 'Resend Code',
        resending: 'Resending...',
        didntReceive: "Didn't receive the code?",
        codeSent: 'Verification code sent to your email',
        codeResent: 'Verification code resent',
        invalidCode: 'Invalid verification code. Please try again.',
        codeRequired: 'Please enter the complete 6-digit code',
        sendingCode: 'Sending verification code...',
        sendCodeFailed: 'Failed to send verification code',
        resendCodeFailed: 'Failed to resend verification code',
        verifyFailed: 'Verification failed',
      },
    },
    danger: {
      dangerZone: 'Danger Zone',
      deleteAccount: 'Delete Account',
      deleteAccountDescription: 'Permanently delete your account and all associated data. This action cannot be undone.',
      deleteAccountConfirmTitle: 'Are you absolutely sure?',
      deleteAccountConfirmDescription: 'This action cannot be undone. This will permanently delete your account and remove your data from our servers.',
      cancel: 'Cancel',
      back: 'Back',
      continue: 'Continue',
      deleting: 'Deleting...',
      accountDeleted: 'Account deleted successfully',
      accountDeleteFailed: 'Failed to delete account',
      // Multi-step delete dialog
      confirmDeletion: 'Confirm Deletion',
      pleaseReadCarefully: 'Please read the following carefully:',
      activePlanCancelsOn: 'You have an active plan that cancels on {date}',
      loseAccessImmediately: 'If you delete today, you will lose access to the app immediately',
      clientsLoseAccess: 'Your clients will lose access to the app immediately',
      dataErased: 'All data will be permanently erased',
      cannotBeUndone: 'This action cannot be undone',
      iUnderstand: 'I Understand',
      typeToConfirm: 'Type to Confirm',
      pleaseType: 'Please type',
      deleteMyAccountQuoted: '"Delete my account"',
      deleteMyAccountPlaceholder: 'Delete my account',
      exactlyToContinue: 'exactly to continue.',
      finalConfirmation: 'Final Confirmation',
      beforeYouProceed: 'Before you proceed:',
      timeLeftOnPlan: 'You still have {time} left on your plan',
      useAppUntil: 'You can still use the app until {date}',
      wontBeBilled: "You won't be billed again",
      sureDeleteToday: 'Are you sure you want to delete your account today?',
      deleteMyAccount: 'Delete My Account',
      // Active subscription dialog
      activeSubscription: 'Active Subscription',
      cancelPlanFirst: 'You have an active subscription. Please cancel your plan first before deleting your account.',
      goToBilling: 'Go to Billing',
      // Active client subscriptions dialog
      activeClientSubscriptions: 'Active Client Subscriptions',
      cancelClientSubscriptionsFirst: 'You have {count} active client subscription(s). Please cancel all client subscriptions in your packages before deleting your account.',
      goToPayments: 'Go to Payments',
    },
    information: {
      accountInformation: 'Account Information',
      accountId: 'Account ID',
      email: 'Email',
      emailVerified: 'Email Verified',
      accountCreated: 'Account Created',
      provider: 'Provider',
      providerGoogle: 'Google',
      providerEmail: 'Email',
      yes: 'Yes',
      no: 'No',
      accountIdCopied: 'Account ID copied to clipboard',
      copyAccountIdAria: 'Copy account ID to clipboard',
      copyFailed: 'Failed to copy to clipboard',
    },
    notifications: {
      savedSuccessfully: 'Notification settings saved successfully',
      saveFailed: 'Failed to save notification settings',
      inApp: 'IN-APP',
      push: 'PUSH',
      bulkActions: 'Bulk Actions',
      turnOffAll: 'Turn off all',
      training: {
        title: 'Training',
        description: 'Notifications related to workout activity.',
        options: {
          workoutCompleted: 'Client completed a workout',
          workoutMissed: 'Client missed a workout',
        },
      },
      forms: {
        title: 'Forms',
        description: 'Notifications when clients submit forms.',
        options: {
          checkinCompleted: 'Client completed a check-in',
          checkinMissed: 'Client missed a check-in',
          questionnaireCompleted: 'Client completed a questionnaire',
        },
      },
      tracking: {
        title: 'Tracking',
        description: 'Notifications related to client tracking activity.',
        options: {
          metricLogged: 'Client logged a metric',
          metricMissed: 'Client missed a metric log',
          habitLogged: 'Client logged a habit',
          habitMissed: 'Client missed a habit log',
          photoUploaded: 'Client uploaded a progress photo',
        },
      },
      profile: {
        title: 'Profile',
        description: 'Notifications related to client profile changes.',
        options: {
          clientConnected: 'Client connected to the app',
          goalAdded: 'Client added a goal',
          goalEdited: 'Client edited a goal',
          goalDeleted: 'Client deleted a goal',
          injuryAdded: 'Client added an injury',
          injuryEdited: 'Client edited an injury',
          injuryDeleted: 'Client deleted an injury',
        },
      },
    },
    company: {
      information: {
        title: 'Company Information',
        logo: 'Company Logo',
        upload: 'Upload',
        uploadAria: 'Upload company logo',
        delete: 'Delete',
        deleteAria: 'Delete company logo',
        companyName: 'Company Name',
        companyNameRequired: 'Company Name *',
        companyNamePlaceholder: 'Enter company name',
        website: 'Website',
        websitePlaceholder: 'Enter website URL',
        linkedin: 'LinkedIn',
        linkedinPlaceholder: 'Enter LinkedIn URL',
        location: 'Location',
        locationPlaceholder: 'Select country',
        specialities: 'Specialities',
        specialitiesPlaceholder: 'Select specialities',
        specialitiesSearchPlaceholder: 'Search specialities...',
        savedSuccessfully: 'Company information saved successfully',
        saveFailed: 'Failed to save company information',
        logoDialog: {
          title: 'Update Company Logo',
          description: 'Upload an image for your company logo',
        },
      },
      team: {
        title: 'Team',
        teamMember: 'TEAM MEMBER',
        role: 'ROLE',
        admin: 'Admin',
        nonAdmin: 'Non admin',
        addTeamMember: 'Add team member',
      },
      branding: {
        title: 'Branding',
      },
    },
    billing: {
      title: 'Billing',
      invoices: {
        title: 'Invoices',
        columns: {
          date: 'Date',
          period: 'Period',
          amount: 'Amount',
          status: 'Status',
          type: 'Type',
          invoice: 'Invoice',
        },
        status: {
          paid: 'Paid',
          open: 'Open',
          draft: 'Draft',
          void: 'Void',
          uncollectible: 'Uncollectible',
        },
        type: {
          subscription: 'Subscription',
          upgrade: 'Upgrade',
        },
        actions: {
          view: 'View',
          pdf: 'PDF',
        },
        empty: 'No invoices yet',
        error: 'Failed to load invoices',
      },
      mobileSuccess: {
        title: 'Subscription Complete!',
        description: 'Your subscription has been updated successfully. Tap Done above to return to the app.',
      },
    },
  },
  sidebar: {
    group: {
      main: 'Main',
      library: 'Library',
      automations: 'Automations',
      business: 'Business',
      athletes: 'Athletes',
    },
    links: {
      dashboard: 'Dashboard',
      marketing: 'Marketing',
      inbox: 'Inbox',
      workouts: 'Workouts',
      athletes: 'Athletes',
      calendar: 'Calendar',
      getStarted: 'Get Started',
      home: 'Home',
      todo: 'To do',
      library: 'Library',
      training: 'Training',
      metrics: 'Metrics',
      profile: 'Profile',
      files: 'Files',
      habits: 'Habits',
      forms: 'Forms',
      nutrition: 'Nutrition',
      onboarding: 'Onboarding',
      flows: 'Flows',
      checkIns: 'Check Ins',
      referAndEarn: 'Refer and Earn',
      getFreeMonth: 'Get a free month',
      packages: 'Packages',
      activity: 'Activity',
    },
    actions: {
      keepMenuOpen: 'Keep menu open',
      keepMenuOpenAria: 'Keep menu open',
      closeSidebar: 'Close sidebar',
      closeSidebarAria: 'Close sidebar',
      openSidebar: 'Open sidebar',
      openSidebarAria: 'Open sidebar',
    },
    footer: {
      addTeamMembers: 'Add team members',
      comingSoon: 'Coming soon',
    },
    profile: {
      openAccountMenuAria: 'Open account menu',
      account: 'Account',
      helpArticles: 'Help Articles',
      billing: 'Billing',
      logOut: 'Log out',
      confirmLogoutTitle: 'Confirm log out',
      confirmLogoutDescription: 'Are you sure you want to log out?',
    },
    search: {
      placeholder: 'Search across Athli...',
      emptyMessage: 'Search for anything and everything...',
      noResults: 'No results found for',
      athletes: 'Athletes',
      workouts: 'Workouts',
      programs: 'Programs',
      exercises: 'Exercises',
      files: 'Files',
      forms: 'Forms',
      habits: 'Habits',
      flows: 'Flows',
      messages: 'Messages',
      exercisesCount: 'exercises',
      aiAssistant: 'AI Assistant',
      aiAssistantAria: 'AI Assistant',
      aiAssistantComingSoon: 'AI Assistant is coming soon!',
      openProfileAria: 'Open profile for {name}',
      openWorkoutAria: 'Open workout {name}',
      openProgramAria: 'Open program {name}',
      openExerciseAria: 'Open exercise {name}',
      openFileAria: 'Open file {name}',
      openFormAria: 'Open form {name}',
      openHabitAria: 'Open habit {name}',
      openFlowAria: 'Open flow {name}',
      openConversationAria: 'Open conversation with {name}',
      sections: 'Sections',
      checkIns: 'Check-ins',
      questionnaires: 'Questionnaires',
      metrics: 'Metrics',
      todos: "To-do's",
      todosYourList: 'To Do - Your List',
      todosAthliAssistant: 'To Do - Athli Assistant',
      conversations: 'Messages',
      showMore: 'Show more',
      showLess: 'Show less',
    },
    theme: {
      label: 'Theme',
      lightMode: 'Light mode',
      darkMode: 'Dark mode',
      toggleAria: 'Toggle theme',
      light: 'Light',
      dark: 'Dark',
      system: 'System',
      preset: 'Theme Preset',
    },
    color: {
      label: 'Theme',
      default: 'Default',
    },
    language: {
      label: 'English',
      aria: 'Change language (current: English)',
    },
    settings: {
      label: 'Settings',
    },
    featureRequests: {
      label: 'Feature Requests',
    },
    helpAndSupport: {
      label: 'Help and support',
    },
    support: {
      label: 'Support',
    },
    help: {
      aria: 'Help',
    },
    logout: {
      loggingOut: 'Logging out...',
    },
  },
  greetings: {
    goodMorning: 'Good morning',
    goodAfternoon: 'Good afternoon',
    goodEvening: 'Good evening',
  },
  general: {
    help: 'Help',
    save: 'Save',
    saving: 'Saving',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    remove: 'Remove',
    add: 'Add',
    change: 'Change',
    copy: 'Copy',
    download: 'Download',
    paste: 'Paste',
    back: 'Back',
    close: 'Close',
    open: 'Open',
    submit: 'Submit',
    reset: 'Reset',
    confirm: 'Confirm',
    confirmDeletion: 'Confirm Deletion',
    confirmArchive: 'Confirm Archive',
    clearSelected: 'Clear {count} selected',
    bulkDeleteConfirmation: 'Are you sure you want to delete {count} selected {item}? This action cannot be undone.',
    deleteSuccess: 'Deleted successfully',
    deleteSuccessName: '{name} deleted successfully',
    deleteSuccessCount: '{count} {item} deleted successfully',
    deleteError: 'Failed to delete. Please try again.',
    assign: 'Assign',
    yes: 'Yes',
    no: 'No',
    loading: 'Loading',
    loadingMore: 'Loading...',
    alreadyUsed: 'Already used',
    uploading: 'Uploading...',
    error: 'Error',
    errorOccurred: 'An error occurred. Please try again.',
    success: 'Success',
    warning: 'Warning',
    info: 'Info',
    search: 'Search',
    filter: 'Filter',
    sort: 'Sort',
    clear: 'Clear',
    clearSelection: 'Clear selection',
    apply: 'Apply',
    done: 'Done',
    next: 'Next',
    previous: 'Previous',
    select: 'Select',
    continue: 'Continue',
    new: 'New',
    selectAll: 'Select all',
    choose: 'Choose',
    create: 'Create',
    update: 'Update',
    view: 'View',
    show: 'Show',
    hide: 'Hide',
    more: 'More',
    less: 'Less',
    all: 'All',
    none: 'None',
    difficulty: 'Difficulty',
    profile: 'Profile',
    description: 'Description',
    type: 'Type',
    equipment: 'Equipment',
    created: 'Created',
    editDetails: 'Edit details',
    comments: 'Comments',
    active: 'Active',
    archived: 'Archived',
    today: 'Today',
    yesterday: 'Yesterday',
    deleteSection: 'Delete Section',
    deleteSectionDescription: 'Are you sure you want to delete this section? This action cannot be undone.',
  },
  messages: {
    title: 'Inbox',
    search: 'Search',
    conversation: 'conversation',
    conversations: 'conversations',
    broadcast: 'Broadcast',
    comingSoon: 'Coming soon!',
    searchPlaceholder: 'Search chats...',
    sortUnread: 'Unread',
    sortRead: 'Read',
    sortOldest: 'Oldest',
    newMessage: 'New message',
    athlete: 'Athlete',
    draft: 'DRAFT',
    noContactsFound: 'No contacts found.',
    loadingConversation: 'Opening conversation...',
    loadingMessages: 'Fetching your messages...',
    viewTrainingCalendar: 'View training calendar',
    training: 'Training',
    viewProfile: 'View {name}\'s profile',
    dropFilesHere: 'Drop your PDFs, videos or images here',
    filesWillBeAdded: 'Files will be added to the message composer',
    selectAthleteToContinue: 'Select an athlete to continue messaging or start a new chat',
    notes: 'Notes',
    note: 'Note',
    createdDate: 'Created Date',
    action: 'Action',
    loadingNotes: 'Loading notes...',
    noNotesFound: 'No notes found',
    searchNotes: 'Search notes',
    searchNotesPlaceholder: 'Search notes...',
    createNote: 'Create note',
    viewNote: 'View note',
    noteTitle: 'Title',
    noteBody: 'Body',
    enterNoteTitle: 'Enter note title...',
    writeNoteHere: 'Write your note here...',
    createdAt: 'Created at',
    updatedAt: 'Updated at',
    reply: 'Reply',
    deleteImages: 'Delete images',
    deleteAllImages: 'Delete all images',
    deletePdf: 'Delete PDF',
    deleteVideo: 'Delete video',
    deleteMessage: 'Delete message',
    deletedMessage: 'Deleted message',
    yourself: 'Yourself',
    removePdf: 'Remove PDF',
    removeVideo: 'Remove video',
    cancelReply: 'Cancel reply',
    uploadFile: 'Upload file',
    uploadImage: 'Upload image',
    uploadPdf: 'Upload PDF',
    uploadVideo: 'Upload video',
    attachFile: 'Attach file',
    attachFiles: 'Attach files',
    typeMessagePlaceholder: 'Leave a comment!',
    typeMessage: 'Type a message',
    sendMessage: 'Send message',
    images: 'Images',
    pdfs: 'PDFs',
    videos: 'Videos',
    video: 'Video',
    pdf: 'PDF',
    mp4: 'MP4',
    voiceNote: 'Voice note',
    download: 'Download {name}',
    remove: 'Remove {name}',
    viewNoteAria: 'View note: {title}',
    closePowerView: 'Close power view',
    enablePowerMode: 'Enable power mode',
    showAthlete: 'Show athlete',
    hideAthlete: 'Hide athlete',
    broadcastPanel: {
      title: 'Broadcast Message',
      message: 'Message',
      messagePlaceholder: 'Enter your message...',
      attachments: 'Attachments',
      selectClients: 'Select Clients',
      confirmClients: 'Confirm Clients',
      confirmMessage: 'Confirm Message',
      selectedClients: 'Selected Clients',
      allClients: 'All clients',
      recipients: 'Recipients',
      clientCount: '{count} {count, plural, =1 {client} other {clients}}',
      searchClients: 'Search clients...',
      noClientsFound: 'No clients found.',
      removeClient: 'Remove {name}',
      broadcastToAllClients: 'Broadcast to All clients',
      broadcastToClients: 'Broadcast to {count} {count, plural, =1 {client} other {clients}}',
    },
  },
  library: {
    title: 'Training',
    workouts: 'Workouts',
    sections: {
      title: 'Sections',
      section: 'Section',
      sectionName: 'Section Name',
      type: 'Type',
      description: 'Description',
      sectionType: 'Section Type',
      exercises: 'Exercises',
      created: 'Created',
      searchPlaceholder: 'Search...',
      sectionNamePlaceholder: 'Name...',
      selectTypePlaceholder: 'Select section type',
      selectSectionType: 'Select Section Type',
      selectSectionTypeDescription: 'Choose the type of section you want to create with AI.',
      descriptionPlaceholder: 'Add notes about this section...',
      emptyMessage: 'No sections found',
      emptyState: {
        title: 'No sections yet',
        subtitle: 'Create your first reusable section to get started',
        action: 'Create Section',
      },
      actions: {
        save: 'Save',
        newSection: 'New Section',
        duplicate: 'Duplicate',
        duplicateAria: 'Duplicate section',
        starAria: 'Star section',
        deleteAria: 'Delete section',
        starSelected: 'Star',
        starSelectedAria: 'Star selected sections',
        clearSelected: 'Clear {count} selected',
        deleteSelected: 'Delete',
      },
      tooltips: {
        description: 'Brief overview of the section',
        sectionType: 'Type of section',
        totalExercises: 'Number of exercises in this section',
        created: 'Date created',
      },
      sorting: {
        ascending: 'Sort Ascending',
        descending: 'Sort Descending',
      },
      toast: {
        deleting: 'Deleting section...',
        deletedSuccessfully: 'Section deleted successfully',
        deletedBulkSuccessfully: '{count} sections deleted successfully',
        duplicatedSuccessfully: 'Section "{name}" duplicated successfully',
        starredSuccessfully: 'Section starred successfully',
        starredWithNameSuccessfully: '{name} starred successfully',
        unstarredSuccessfully: 'Section unstarred successfully',
        unstarredWithNameSuccessfully: '{name} unstarred successfully',
        savedSuccessfully: 'Section "{name}" saved successfully',
        updatedSuccessfully: 'Section "{name}" updated successfully',
        failedToSave: 'Failed to save section',
        failedToLoad: 'Failed to load section',
        failedToDelete: 'Failed to delete section',
      },
    },
    programs: 'Programs',
    exercises: 'Exercises',
    exercisesComingSoon: 'Exercises page coming soon!',
    marketplace: 'Marketplace',
    openMarketplace: 'Open marketplace',
    marketplaceComingSoon: 'Coming soon! Browse pre-made workouts and programs, sell your own programs and more!',
    week: 'Week',
    day: 'Day',
    // Legacy keys for workouts page - should be migrated to workouts.*
    workoutColumn: 'Workout',
    briefOverviewWorkout: 'A brief overview of the workout program',
    categoryStyleWorkout: 'The category or style of the workout program',
    totalExercises: 'Total Exercises',
    numberOfExercisesWorkout: 'The number of exercises in the workout program',
    equipmentRequiredWorkout: 'The equipment required for this workout program',
    dateCreatedWorkout: 'The date when the workout program was created',
    starred: 'Starred',
    unstarred: 'Unstarred',
    assign: 'Assign',
    assignWorkout: 'Assign workout',
    assignWorkoutToClient: 'Assign this workout to a client',
    starWorkout: 'Star this workout',
    selectAllWorkouts: 'Select all workouts',
    sortAscending: 'Sort ascending',
    sortDescending: 'Sort descending',
    createWorkout: 'Create workout',
    newWorkout: 'New workout',
    noWorkoutsFound: 'No workouts found.',
    searchPlaceholder: 'Search...',
    workout: 'workout',
    workoutPlural: 'workouts',
    continue: 'Save',
    generate: 'Generate',
    generateWorkout: 'Generate workout',
    backToWorkoutDetails: 'Back to workout details',
    cancelCreatingWorkout: 'Cancel creating workout',
    cancel: 'Cancel',
    dropPdfHere: 'Drop your PDF here',
    pdfFilesOnly: 'PDF files only',
    athliAiBuilder: 'Athli AI Builder',
    AthliAiBuilder: 'Athli AI Builder',
    dragDropPdf: 'Drag and drop or select PDF files to instantly import into Athli.',
    letsBuildWorkout: 'Let\'s build a workout',
    letsBuildSection: 'Let\'s build a section',
    workoutPromptPlaceholder: 'Ask for an auto-made workout, explain what you want to be included in yours and write in whatever form you wish. Press Enter to add new lines.',
    sectionPromptPlaceholder: 'Describe the section you want to create — exercises, format, intensity, and any preferences.',
    refineWorkout: 'Refine your workout...',
    refineSection: 'Refine your section...',
    generatingWorkout: 'Generating workout...',
    generatingSection: 'Generating section...',
    sectionTitle: 'Section title',
    selectPdfFile: 'Select PDF file',
    useExample: 'Use our example',
    removePdfFile: 'Remove PDF file',
    assigningWorkout: 'Assigning {name}',
    workoutNameRequired: 'Workout name is required',
    workoutTypeRequired: 'Workout type is required',
    difficultyRequired: 'Difficulty is required',
    workoutCreatedSuccessfully: 'Workout "{name}" created successfully',
    workoutCreationFailed: 'Failed to create workout. Please try again.',
  },
  programs: {
    title: 'Programs',
    subtitle: '{count} {count, plural, =1 {program} other {programs}}',
    emptyMessage: 'No programs found.',
    emptyState: {
      title: 'Build a program',
      subtitle: 'Quickly build programs of any length and type and save them to your library.',
      startCreating: 'Create program',
      startCreatingAria: 'Create a new program',
    },
    searchPlaceholder: 'Search...',
    program: 'program',
    programPlural: 'programs',
    columns: {
      program: 'Program',
      description: 'Description',
      type: 'Type',
      length: 'Length',
      totalWorkouts: 'Workouts',
      created: 'Created',
    },
    columnTooltips: {
      program: 'The name of the program',
      description: 'A brief overview of the program',
      type: 'The category or style of the program',
      difficulty: 'The difficulty level of the program',
      length: 'The duration of the program',
      totalWorkouts: 'The total number of workouts in the program',
      created: 'The date when the program was created',
    },
    actions: {
      createProgram: 'Create program',
      assignProgram: 'Assign program',
      assignProgramToClient: 'Assign this program to a client',
      assignProgramAria: 'Assign program',
      starProgram: 'Star this program',
      starProgramAria: 'Star this program',
      clearSelected: 'Clear selected',
      clearSelectedAria: 'Clear selected programs',
      starSelected: 'Star',
      starSelectedAria: 'Star programs',
      archiveSelected: 'Archive',
      archiveSelectedAria: 'Archive selected programs',
      deleteSelected: 'Delete',
      deleteSelectedAria: 'Delete selected programs',
      duplicate: 'Duplicate',
      duplicateAria: 'Duplicate program',
      export: 'Export',
      exportSelected: 'Export selected',
      exportSelectedAria: 'Export selected programs',
      selectAllPrograms: 'Select all programs',
      sortAscending: 'Sort ascending',
      sortDescending: 'Sort descending',
    },
    filters: {
      type: 'Type',
      show: 'Show',
      starred: 'Starred',
      unstarred: 'Unstarred',
    },
    addProgram: {
      title: 'New program',
      programName: 'Program Name',
      programNameRequired: 'Program Name *',
      programNamePlaceholder: 'Name...',
      type: 'Type',
      typeRequired: 'Type *',
      typePlaceholder: 'Select...',
      difficulty: 'Difficulty',
      difficultyRequired: 'Difficulty *',
      difficultyPlaceholder: 'Select...',
      weeks: 'Weeks',
      weeksPlaceholder: 'Number of weeks',
      description: 'Description',
      descriptionOptional: '(Optional)',
      descriptionPlaceholder: 'Add a description for your program...',
      continue: 'Save',
      continueAria: 'Save program',
      cancel: 'Cancel',
      cancelAria: 'Cancel creating program',
      programNameRequiredError: 'Program name is required',
      programTypeRequiredError: 'Program type is required',
      difficultyRequiredError: 'Difficulty is required',
    },
    new: {
      toast: {
        savedSuccessfully: 'Program "{name}" saved successfully',
      },
    },
    assigning: {
      title: 'Assigning {name}',
      titleGeneric: 'Assign program',
    },
    detail: {
      breadcrumb: {
        library: 'Library',
        programs: 'Programs',
        program: 'Program',
      },
      title: 'Program',
      edit: 'Edit',
      editAria: 'Edit program',
      delete: 'Delete',
      deleteAria: 'Delete program',
      deleteModal: {
        title: 'Delete',
        description: 'Are you sure you want to delete this program? This action cannot be undone.',
        cancel: 'Cancel',
        confirm: 'Delete',
        closeAria: 'Close',
      },
      toast: {
        deletedSuccessfully: 'Program deleted successfully',
        starredSuccessfully: '{name} starred successfully',
        unstarredSuccessfully: '{name} unstarred successfully',
      },
    },
    descriptionModal: {
      title: '{name} - Description',
    },
    delete: {
      toast: {
        success: 'Program deleted successfully',
      },
    },
    edit: {
      toast: {
        updatedSuccessfully: 'Program "{name}" updated successfully',
      },
    },
    builder: {
      newProgram: 'New program',
      editProgram: 'Edit program',
      save: 'Save',
      saveAria: 'Save program',
      cancel: 'Cancel',
      cancelAria: 'Cancel editing program',
      previousWeek: 'Previous week',
      previousWeekAria: 'Previous week',
      nextWeek: 'Next week',
      nextWeekAria: 'Next week',
      addWeek: 'Add week',
      addWeekAria: 'Add week',
      duplicateWeek: 'Duplicate week',
      duplicateWeekAria: 'Duplicate week',
      clearWeek: 'Clear week',
      clearWeekAria: 'Clear week',
      clearWeekConfirmTitle: 'Clear week {number}?',
      clearWeekConfirmDescription: 'Are you sure you want to clear all workouts from week {number}? This action cannot be undone.',
      clearWeekAction: 'Clear',
      deleteWeek: 'Delete week',
      deleteWeekAria: 'Delete week',
      undo: 'Undo',
      undoAria: 'Undo',
      redo: 'Redo',
      redoAria: 'Redo',
      weekLabel: 'Week {number}',
      dayLabel: 'Day {number}',
      weekRange: 'Week {start} of {total}',
      weekRangeMultiple: 'Week {start}-{end} of {total}',
      overview: 'Overview',
      selectRepeatEveryDays: 'Select repeat every days option',
      selectRepeatWeekly: 'Select repeat weekly option',
      showOneWeek: 'Show one week',
      showTwoWeeks: 'Show two weeks',
      twoWeeksRequirement: 'Add at least 2 weeks to view two weeks',
      showFourWeeks: 'Show four weeks',
      fourWeeksRequirement: 'Add at least 4 weeks to view four weeks',
      viewDetailsForWorkout: 'View details for workout {name}',
      selectWorkout: 'Select workout {name}',
      months: {
        jan: 'Jan',
        feb: 'Feb',
        mar: 'Mar',
        apr: 'Apr',
        may: 'May',
        jun: 'Jun',
        jul: 'Jul',
        aug: 'Aug',
        sep: 'Sep',
        oct: 'Oct',
        nov: 'Nov',
        dec: 'Dec',
      },
      addWorkout: {
        title: 'Add workout',
        titleDay: 'Add Workout - Day {day}',
        searchPlaceholder: 'Search workouts...',
        noWorkoutsFound: 'No workouts found.',
        scheduleOption: {
          once: 'Add only for this day',
          everyDays: 'Add this workout every',
          weekly: 'Repeat weekly on day',
        },
        days: 'days',
        numberOfDays: 'Number of days',
        dayOfWeek: 'Day of week',
        add: 'Add',
        addAria: 'Add workout',
        cancel: 'Cancel',
      },
      toast: {
        savedSuccessfully: 'Program saved successfully',
        saveFailed: 'Failed to save program',
        weekDuplicated: 'Week {number} duplicated successfully',
      },
      multiSelect: {
        workoutsSelected: '{count} workout(s) selected',
        clickToAddWorkout: 'Click on a day to add this workout',
        clickToAddWorkouts: 'Click on a day to add this group of workouts',
        deleteAria: 'Delete selected workouts',
        copyAria: 'Copy selected workouts',
        selectWorkoutAria: 'Select workout {name}',
      },
    },
  },
  exercises: {
    title: 'Exercises',
    subtitle: '{count} {count, plural, =1 {exercise} other {exercises}}',
    emptyMessage: 'No exercises found.',
    emptyState: {
      title: 'Create an exercise',
      subtitle: 'Introduce your own custom exercises to reflect your training style',
      startCreating: 'Start creating',
      startCreatingAria: 'Start creating an exercise',
    },
    searchPlaceholder: 'Search...',
    exercise: 'exercise',
    exercisePlural: 'exercises',
    columns: {
      exercise: 'Exercise',
      category: 'Category',
      muscleGroup: 'Muscle Group',
      modality: 'Modality',
      difficulty: 'Difficulty',
      force: 'Force',
      mechanic: 'Mechanic',
      created: 'Created',
    },
    columnTooltips: {
      exercise: 'The name of the exercise',
      category: 'The category of the exercise',
      muscleGroup: 'The muscle group targeted by the exercise',
      modality: 'The modality or method of the exercise',
      difficulty: 'The difficulty level of the exercise',
      force: 'The force type (push/pull)',
      mechanic: 'The mechanic type (isolation/compound)',
      equipment: 'The equipment required for this exercise',
      created: 'The date when the exercise was created',
    },
    actions: {
      createExercise: 'Create exercise',
      assignExercise: 'Assign exercise',
      assignExerciseToClient: 'Assign this exercise to a client',
      assignExerciseAria: 'Assign exercise',
      starExercise: 'Star this exercise',
      starExerciseAria: 'Star this exercise',
      clearSelected: 'Clear selected',
      clearSelectedAria: 'Clear selected exercises',
      starSelected: 'Star',
      starSelectedAria: 'Star exercises',
      archiveSelected: 'Archive',
      archiveSelectedAria: 'Archive selected exercises',
      deleteSelected: 'Delete',
      deleteSelectedAria: 'Delete selected exercises',
      export: 'Export',
      exportSelected: 'Export selected',
      exportSelectedAria: 'Export selected exercises',
      selectAllExercises: 'Select all exercises',
      sortAscending: 'Sort ascending',
      sortDescending: 'Sort descending',
      createSuccess: 'Exercise created successfully',
      updateSuccess: 'Exercise updated successfully',
    },
    detail: {
      toast: {
        starredSuccessfully: '{name} starred successfully',
        unstarredSuccessfully: '{name} unstarred successfully',
      },
    },
    filters: {
      type: 'Type',
      show: 'Show',
      starred: 'Starred',
      unstarred: 'Unstarred',
    },
    addExercise: {
      title: 'New exercise',
      editTitle: 'Edit exercise',
      exerciseName: 'Exercise name',
      exerciseNameRequired: 'Exercise Name *',
      exerciseNamePlaceholder: 'Enter exercise name...',
      instructions: 'Exercise instructions',
      instructionsPlaceholder: 'Enter exercise instructions...',
      instructionsRequiredError: 'Exercise instructions are required',
      video: 'Video',
      videoLink: 'Video link',
      videoLinkPlaceholder: 'YouTube or Vimeo URL',
      videoLinkRequiredError: 'Video link is required',
      videoLinkInvalidError: 'Please enter a valid YouTube or Vimeo URL',
      clearVideoLink: 'Clear video link',
      vimeoThumbnailPlaceholder: 'Vimeo video preview',
      customUpload: 'Custom upload',
      category: 'Category',
      categoryRequiredError: 'Category is required',
      muscleGroups: 'Muscle group',
      muscleGroupsPlaceholder: 'Select muscle groups...',
      searchMuscleGroups: 'Search muscle groups...',
      muscleGroupsRequiredError: 'At least one muscle group is required',
      difficulty: 'Difficulty',
      continue: 'Continue',
      continueAria: 'Continue creating exercise',
      saveAria: 'Save exercise',
      cancel: 'Cancel',
      cancelAria: 'Cancel creating exercise',
      exerciseNameRequiredError: 'Exercise name is required',
      exerciseTypeRequiredError: 'Exercise type is required',
      difficultyRequiredError: 'Difficulty is required',
    },
    assigning: {
      title: 'Assigning {name}',
      titleGeneric: 'Assign exercise',
    },
  },
  workouts: {
    title: 'Workouts',
    subtitle: '{count} {count, plural, =1 {workout} other {workouts}}',
    emptyMessage: 'No workouts found.',
    emptyState: {
      title: 'Build a workout',
      subtitle: 'Build your own custom workouts to match your training style',
      startCreating: 'Create workout',
      startCreatingAria: 'Create a new workout',
    },
    searchPlaceholder: 'Search...',
    workout: 'workout',
    workoutPlural: 'workouts',
    columns: {
      workout: 'Workout',
      description: 'Description',
      type: 'Type',
      totalExercises: 'Total Exercises',
      equipment: 'Equipment',
      created: 'Created',
    },
    columnTooltips: {
      workout: 'The name of the workout',
      description: 'A brief overview of the workout program',
      type: 'The category or style of the workout program',
      difficultyLevel: 'The difficulty level of the workout',
      totalExercises: 'The number of exercises in the workout program',
      equipment: 'The equipment required for this workout program',
      created: 'The date when the workout program was created',
    },
    actions: {
      createWorkout: 'Create workout',
      assignWorkout: 'Assign workout',
      assignWorkoutToClient: 'Assign this workout to a client',
      assignWorkoutAria: 'Assign workout',
      starWorkout: 'Star this workout',
      starWorkoutAria: 'Star this workout',
      clearSelected: 'Clear selected',
      clearSelectedAria: 'Clear selected workouts',
      starSelected: 'Star',
      starSelectedAria: 'Star selected workouts',
      archiveSelected: 'Archive',
      archiveSelectedAria: 'Archive selected workouts',
      deleteSelected: 'Delete',
      deleteSelectedAria: 'Delete selected workouts',
      duplicate: 'Duplicate',
      duplicateAria: 'Duplicate workout',
      export: 'Export',
      exportSelected: 'Export selected',
      exportSelectedAria: 'Export selected workouts',
      selectAllWorkouts: 'Select all workouts',
      sortAscending: 'Sort ascending',
      sortDescending: 'Sort descending',
    },
    filters: {
      type: 'Type',
      show: 'Show',
      starred: 'Starred',
      unstarred: 'Unstarred',
    },
    addWorkout: {
      title: 'New workout',
      workoutName: 'Workout Name',
      workoutNameRequired: 'Workout Name *',
      workoutNamePlaceholder: 'Name...',
      type: 'Type',
      typeRequired: 'Type *',
      typePlaceholder: 'Select...',
      difficulty: 'Difficulty',
      difficultyRequired: 'Difficulty *',
      difficultyPlaceholder: 'Select...',
      description: 'Description',
      descriptionOptional: '(Optional)',
      descriptionPlaceholder: 'Add a description for your workout...',
      selectBuilder: 'Select how you wish to start',
      selectBuilderRequired: 'Select how you wish to start *',
      athliAi: 'Athli AI',
      athliAiDescription: 'AI Workout Builder',
      athliAiAria: 'Use Athli AI to build workout',
      AthliAi: 'Athli AI',
      AthliAiDescription: 'AI Workout Builder',
      AthliAiAria: 'Use Athli AI to build workout',
      standardBuilder: 'Standard Builder',
      standardBuilderDescription: 'Manually build your workout',
      standardBuilderAria: 'Manually build workout',
      continue: 'Continue',
      continueAria: 'Continue creating workout',
      back: 'Back',
      backAria: 'Back to workout details',
      cancel: 'Cancel',
      cancelAria: 'Cancel creating workout',
      openWorkoutDetails: 'Open workout details',
      openSectionDetails: 'Edit section details',
      generate: 'Generate',
      generateAria: 'Generate workout',
      workoutNameRequiredError: 'Workout name is required',
      workoutTypeRequiredError: 'Workout type is required',
      difficultyRequiredError: 'Difficulty is required',
      aiBuilder: {
        title: 'Athli AI Builder',
        description: 'Drag and drop or select PDF files to instantly import into Athli.',
        dropPdfHere: 'Drop your PDF here',
        pdfFilesOnly: 'PDF files only',
        letsBuildWorkout: 'Let\'s build a workout',
        letsBuildWorkoutRequired: 'Let\'s build a workout *',
        promptPlaceholder: 'Ask for an auto-made workout, explain what you want to be included in yours and write in whatever form you wish. Press Enter to add new lines.',
        useExample: 'Use our example',
        useExampleAria: 'Use our example',
        selectPdfFile: 'Select PDF file',
        selectPdfFileAria: 'Select PDF file',
        removePdfFile: 'Remove PDF file',
        removePdfFileAria: 'Remove PDF file',
      },
    },
    deleteWorkout: {
      title: 'Delete workout',
      description: 'Are you sure you want to delete this workout? This action cannot be undone.',
    },
    assigning: {
      title: 'Assigning {name}',
      titleGeneric: 'Assign workout',
    },
    descriptionModal: {
      editWorkout: 'Edit workout',
      editWorkoutAria: 'Edit workout',
      close: 'Close',
      closeAria: 'Close workout preview',
      overview: 'Overview',
    },
    detail: {
      breadcrumb: {
        library: 'Library',
        workouts: 'Workouts',
        workout: 'Workout',
      },
      title: 'Workout',
      edit: 'Edit',
      editAria: 'Edit workout',
      editOptions: {
        AthliAi: 'Athli AI',
        manual: 'Manual',
        editOptionsAria: 'Edit options',
      },
      delete: 'Delete',
      deleteAria: 'Delete workout',
      deleteModal: {
        title: 'Delete',
        description: 'Are you sure you want to delete this workout? This action cannot be undone.',
        cancel: 'Cancel',
        confirm: 'Delete',
        closeAria: 'Close',
      },
      toast: {
        deletedSuccessfully: 'Workout deleted successfully',
        deletedBulkSuccessfully: '{count} workouts deleted successfully',
        duplicatedSuccessfully: 'Workout "{name}" duplicated successfully',
        archivedSuccessfully: 'Workout archived successfully',
        unarchivedSuccessfully: 'Workout restored successfully',
        starredSuccessfully: '{name} starred successfully',
        starredBulkSuccessfully: '{count} workouts starred successfully',
        unstarredSuccessfully: '{name} unstarred successfully',
      },
    },
    new: {
      toast: {
        savedSuccessfully: 'Successfully saved',
        failedToSave: 'Failed to save workout',
      },
    },
    edit: {
      breadcrumb: {
        library: 'Library',
        workouts: 'Workouts',
        editWorkout: 'Edit workout',
      },
      title: 'Edit workout',
      save: 'Save',
      saveAria: 'Save workout',
      cancel: 'Cancel',
      cancelAria: 'Cancel editing workout',
      toast: {
        updatedSuccessfully: 'Workout "{name}" updated successfully',
        failedToUpdate: 'Failed to update workout',
      },
    },
    discardChanges: {
      title: 'Discard changes?',
      description: 'Changes have not been saved yet.',
      cancel: 'Cancel',
      confirm: 'Discard',
    },
  },
  calendar: {
    title: 'Calendar',
    checkingConnection: 'Checking calendar connection...',
    failedToCheckStatus: 'Failed to check calendar status',
    connectYourCalendar: 'Connect your Calendar',
    connectDescription: 'You will be able to book sessions directly here and it will sync with your connected calendar.',
    connected: 'Calendar Connected',
    connectedDescription: 'Your calendar is connected and ready to use.',
    gmail: 'Gmail',
    outlook: 'Outlook',
    connectCalendar: 'Connect Calendar',
    loadingEvents: 'Loading calendar events...',
    today: 'Today',
    goToToday: 'Go to today',
    previousPeriod: 'Previous period',
    nextPeriod: 'Next period',
    week: 'Week',
    month: 'Month',
    bookMeeting: 'Book a meeting',
    bookMeetingAria: 'Book a meeting',
    days: {
      mon: 'Mon',
      tue: 'Tue',
      wed: 'Wed',
      thu: 'Thu',
      fri: 'Fri',
      sat: 'Sat',
      sun: 'Sun',
    },
    time: {
      am12: '12AM',
      pm12: '12PM',
    },
    meeting: {
      add: 'Add',
      title: 'Title',
      titleRequired: 'Title *',
      titlePlaceholder: 'Meeting title',
      invite: 'Invite',
      inviteRequired: 'Invite *',
      searchAthletes: 'Search athletes...',
      date: 'Date',
      dateRequired: 'Date *',
      selectDate: 'Select date',
      location: 'Location',
      locationRequired: 'Location *',
      selectLocation: 'Select location',
      online: 'Online',
      inPersonGym: 'In person (Gym)',
      startTime: 'Start time',
      startTimeRequired: 'Start time *',
      endTime: 'End time',
      endTimeRequired: 'End time *',
      description: 'Description',
      descriptionPlaceholder: 'Meeting description',
    },
    event: {
      edit: 'Edit meeting',
      editAria: 'Edit meeting',
      email: 'Email meeting',
      emailAria: 'Email meeting',
      delete: 'Delete meeting',
      deleteAria: 'Delete meeting',
      closeAria: 'Close',
      noDateSpecified: 'No date specified',
      untitled: 'Untitled Meeting',
      invalidDateTime: 'Invalid date/time',
      clients: 'Clients',
      enterClientEmails: 'Enter client email addresses',
      searchOrEnterEmail: 'Search or enter email...',
    },
    email: {
      title: 'Email client',
      clients: 'Clients',
      clientsRequired: 'Clients *',
      searchByName: 'Search by name',
      subject: 'Subject',
      subjectRequired: 'Subject *',
      subjectPlaceholder: 'Subject..',
      message: 'Message',
      messagePlaceholder: 'Message...',
      appointmentDetailsIncluded: 'Appointment details will be included in the email.',
      send: 'Send',
    },
    toast: {
      meetingDeleted: 'Meeting deleted successfully',
      failedToDelete: 'Failed to delete meeting',
      meetingUpdated: 'Meeting updated successfully',
      failedToUpdate: 'Failed to update meeting',
      emailSent: 'Email sent successfully',
      failedToSendEmail: 'Failed to send email',
      connectedSuccessfully: 'Calendar connected successfully!',
      connectionFailed: 'Failed to connect calendar',
    },
    connect: {
      title: 'Connect',
      connectProvider: 'Connect {provider}',
      onlyOneCalendar: 'Only one calendar per user can be connected.',
      description: 'Only one Google or Microsoft (365, Outlook) calendar per user can be connected.',
      emailAddress: 'Email address',
      emailAddressRequired: 'Email address *',
      emailPlaceholder: 'your.email@example.com',
      emailAria: 'Email address for calendar connection',
      detectingProvider: 'Detecting calendar provider...',
      unableToDetect: 'Unable to detect calendar provider. Please ensure your email uses Google Workspace or Microsoft 365.',
      providerMismatch: 'The email address you entered is linked to a different provider: {detected}, but you selected {selected}. Please use the corrent providers email address.',
      cancel: 'Cancel',
      cancelAria: 'Cancel calendar connection',
      connect: 'Connect Calendar',
      connectAria: 'Connect calendar',
      connecting: 'Connecting...',
      checking: 'Checking...',
      connectGoogle: 'Connect Google',
      connectOutlook: 'Connect Outlook',
      connectIcloud: 'Connect iCloud',
      pleaseEnterEmail: 'Please enter a valid email address',
      unableToDetectProvider: 'Unable to detect calendar provider. Please check your email address.',
      icloudNotAvailable: 'iCloud calendar integration is not yet available. Please use Google or Outlook calendar.',
    },
    callback: {
      connecting: 'Connecting your calendar...',
      connectionFailed: 'Failed to connect calendar',
      noTokensReturned: 'No tokens returned from OAuth provider',
      calendarConnectionFailed: 'Calendar connection failed. No access token returned.',
      failedToSave: 'Failed to save calendar connection',
    },
  },
  athletes: {
    title: 'Athletes',
    subtitle: '{count} {count, plural, =1 {client} other {clients}}',
    emptyMessage: 'No athletes found.',
    emptyState: {
      title: 'Invite your clients',
      subtitle: 'Manage all of your clients through Athli',
    },
    searchPlaceholder: 'Search...',
    columns: {
      lastActivity: 'Last activity',
      last7DaysTraining: 'L7D Training',
      last30DaysTraining: 'L30D Training',
      category: 'Category',
      status: 'Status',
      email: 'Email',
      phone: 'Phone',
      country: 'Country',
      birthDate: 'Birth Date',
      age: 'Age',
      clientFor: 'Client For',
      athlete: 'Athlete',
    },
    columnTooltips: {
      lastActivity: 'Time since last logged on to app',
      last7DaysTraining: 'How much they trained in the last 7 days out of their total assigned schedule',
      last30DaysTraining: 'How much they trained in the last 30 days out of their total assigned schedule',
      category: 'Whether they are online, in-person, or hybrid',
      status: 'Whether the client is invited, accepted, or connected via the app',
      clientFor: 'How long they have been a client',
    },
    status: {
      connected: 'Connected',
      accepted: 'Accepted',
      invited: 'Invited',
    },
    filters: {
      category: 'Category',
      status: 'Status',
      online: 'Online',
      inPerson: 'In-person',
      hybrid: 'Hybrid',
      uncategorized: 'Not set',
    },
    actions: {
      addClient: 'Add Client',
      uploadClients: 'Upload clients',
      singleClient: 'Single client',
      yourInviteLink: 'Your invite link',
      copyInviteLink: 'Copy invite link',
      selectAll: 'Select all athletes',
      sortAscending: 'Sort ascending',
      sortDescending: 'Sort descending',
      messageClient: 'Message the client',
      messageAria: 'Message {name}',
      viewTrainingCalendar: 'View clients training calendar',
      clearSelected: 'Clear selected',
      clearSelectedAria: 'Clear selected athletes',
      export: 'Export',
      exportSelectedAria: 'Export selected athletes',
      archive: 'Archive',
      archivePlural: 'Archive {item}',
      archivePluralTooltip: 'Archive {item} that are active',
      archiveSelectedAria: 'Archive selected athletes',
      viewTrainingCalendarAria: 'Open training calendar for {name}',
      copyAthleteName: 'Copy athlete name',
      copyAthleteNameAria: 'Copy {name}',
      copyEmail: 'Copy email for {name}',
      copyPhone: 'Copy phone for {name}',
      revealEmail: 'Reveal email for {name}',
      hideEmail: 'Hide email for {name}',
      revealPhone: 'Reveal phone for {name}',
      hidePhone: 'Hide phone for {name}',
      restoreClient: 'Restore client',
      uploadCSV: 'Upload CSV',
      archivedClients: 'Restore archived clients',
    },
    export: {
      name: 'Name',
      email: 'Email',
      phone: 'Phone',
      country: 'Country',
      category: 'Category',
      status: 'Status',
      lastActivity: 'Last Activity',
      last7DaysTraining: 'Last 7 Days Training',
      last30DaysTraining: 'Last 30 Days Training',
      age: 'Age',
      birthDate: 'Birth Date',
      clientFor: 'Client For',
    },
    timeUnits: {
      year: 'year',
      years: 'years',
      day: 'day',
      days: 'days',
    },
    addClient: {
      title: 'Add athlete',
      fullName: 'Full Name',
      fullNameRequired: 'Full Name *',
      fullNamePlaceholder: 'Enter full name',
      email: 'Email',
      emailRequired: 'Email *',
      emailPlaceholder: 'Enter email address',
      coachingType: 'Coaching Type',
      coachingTypeRequired: 'Coaching Type *',
      online: 'Online',
      inPerson: 'In-person',
      hybrid: 'Hybrid',
      onlineAria: 'Online coaching',
      inPersonAria: 'In-person coaching',
      hybridAria: 'Hybrid coaching',
      sendInvitation: 'Send Invitation',
      sendInvitationAria: 'Send invitation',
      cancelAria: 'Cancel adding athlete',
      fullNameRequiredError: 'Full name is required',
      emailInvalidError: 'Please enter a valid email address',
      onboardingFlow: 'Onboarding flow',
      invitationSent: 'Invitation sent successfully',
      invitationSentDescription: 'An invitation has been sent to {fullName} at {email}',
    },
    inviteDialog: {
      title: 'Your Invite Link',
      askDescription: 'Would you like to add a specific onboarding flow to this invite link?',
      copyDescription: 'Copy this link and share it with your client.',
      noJustCopy: 'No, just copy link',
      yesSelect: 'Yes, select onboarding',
      selectOnboarding: 'Select an onboarding flow',
      copyLink: 'Copy link',
      linkCopied: 'Invite link copied!',
    },
    uploadClients: {
      title: 'Upload clients',
      step1: 'Upload CSV',
      step2: 'Review Info',
      save: 'Save',
      continue: 'Continue',
      continueToReview: 'Continue to review clients',
      continueToFinal: 'Continue to final step',
      cancel: 'Cancel',
      cancelAria: 'Cancel uploading clients',
      validating: 'Validating...',
      changeFile: 'Change File',
      clickToUpload: 'Click to upload or drag and drop',
      csvFileOnly: 'CSV file only',
      selectFile: 'Select File',
      downloadTemplate: 'Please download this',
      csvTemplate: 'CSV Template',
      addClientInfo: ', add your client info and then upload.',
      reviewTitle: 'Review {term}',
      fullName: 'Full Name',
      email: 'Email',
      category: 'Category',
      noClientsFound: 'No clients found',
      removeClient: 'Remove {name}',
      invalidCsv: 'Invalid CSV file',
      errorProcessing: 'Error processing CSV file',
      onboardingTitle: 'Onboarding flow',
      onboardingDescription: 'Optionally select an onboarding flow for these clients.',
      send: 'Send',
    },
    restorePanel: {
      title: 'Restore Client',
      subtitle: 'Select an archived client to restore',
      searchPlaceholder: 'Search archived clients...',
      emptyState: 'No archived clients found',
      restoreButton: 'Restore',
      columns: {
        name: 'Name',
        email: 'Email',
      },
    },
    notifications: {
      archiveSuccess: 'Client archived successfully',
      archiveError: 'Failed to archive client',
      restoreSuccess: 'Client restored successfully',
      restoreError: 'Failed to restore client',
    },
    profile: {
      athletes: 'Athletes',
      overview: 'Overview',
      notes: 'Notes',
      metrics: 'Metrics',
      checkIn: 'Check Ins',
      forms: 'Forms',
      breadcrumb: {
        overview: 'Overview',
      },
      checkIns: {
        title: 'Check-ins',
        searchPlaceholder: 'Search check-ins...',
        addCheckIn: 'Add Check-in',
        addCheckInAria: 'Add check-in to athlete',
        emptyMessage: 'No check-ins found.',
        clearSelected: 'Clear',
        publish: 'Publish',
        pause: 'Pause',
        resume: 'Resume',
        publishConfirmTitle: 'Publish check-in?',
        publishConfirmDescription: 'Are you sure? This will make this check-in live and the client will start receiving it according to the schedule.',
        publishSuccess: 'Check-in published successfully',
        pauseConfirmTitle: 'Pause check-in?',
        pauseConfirmDescription: 'Are you sure? This will pause the check-in and the client will stop receiving it until you resume.',
        pauseSuccess: 'Check-in paused successfully',
        resumeConfirmTitle: 'Resume check-in?',
        resumeConfirmDescription: 'Are you sure? This will resume the check-in and the client will start receiving it again according to the schedule.',
        resumeSuccess: 'Check-in resumed successfully',
        columns: {
          name: 'Name',
          status: 'Status',
          schedule: 'Schedule',
          submissions: 'Submissions',
          createdAt: 'Created on',
          nextScheduledAt: 'Next scheduled',
          description: 'Description',
        },
        questions: '{count} question',
        questionsPlural: '{count} questions',
        goToForms: 'Go to Forms',
        noFormsMessage: 'You have not created a form yet. Please navigate to',
        formsLink: 'Forms',
        status: {
          draft: 'Draft',
          live: 'Live',
          paused: 'Paused',
          assigned: 'Assigned',
          completed: 'Completed',
          review: 'Review',
          reviewed: 'Reviewed',
        },
        reviewTitle: 'Review check-in',
        reviewCompletedTitle: 'Review completed',
        reviewPlaceholder: 'Enter your review...',
        completeReview: 'Complete review',
        completingReview: 'Completing review...',
        editReview: 'Edit',
        updateReview: 'Update review',
        updatingReview: 'Updating review...',
        schedule: {
          frequency: {
            label: 'Frequency',
            daily: 'Daily',
            weekly: 'Weekly',
            biweekly: 'Biweekly',
            monthly: 'Monthly',
          },
          selectDays: 'Select days',
          selectDay: 'Select day',
          dayOfMonth: 'Day of the month',
          monthly: {
            first: '1st of the month',
            last: 'Last of the month',
            specific: 'Specific day',
            selectDay: 'Select day',
          },
          explanation: {
            daily: 'This check-in will be sent daily on {days} once assigned to an athlete.',
            weekly: 'This check-in will be sent weekly on {day} once assigned to an athlete.',
            biweekly: 'This check-in will be sent biweekly on {day} once assigned to an athlete.',
            monthlyFirst: 'This check-in will be sent monthly on the 1st of each month once assigned to an athlete.',
            monthlyLast: 'This check-in will be sent monthly on the last day of each month once assigned to an athlete.',
            monthlySpecific: 'This check-in will be sent monthly on the {day} of each month once assigned to an athlete.',
          },
          reminder: 'This check-in will be scheduled as:',
        },
        noSubmissions: 'No submissions yet',
        selectSubmission: 'Select a submission to view',
      },
      questionnaires: {
        title: 'Questionnaires',
        searchPlaceholder: 'Search questionnaires...',
        addQuestionnaire: 'Add Questionnaire',
        addQuestionnaireAria: 'Add questionnaire to athlete',
        emptyMessage: 'No questionnaires found.',
        clearSelected: 'Clear',
        columns: {
          name: 'Name',
          status: 'Status',
          sentAt: 'Sent at',
          completedAt: 'Completed at',
          description: 'Description',
        },
        status: {
          draft: 'Draft',
          pending: 'Pending',
          completed: 'Completed',
        },
        questions: '{count} question',
        questionsPlural: '{count} questions',
        sendForm: 'Send form',
        sendAgain: 'Send again',
        sendConfirmTitle: 'Send questionnaire?',
        sendConfirmDescription: 'Are you sure? This will send this form immediately to the client and cannot be undone.',
        sendSuccess: 'Questionnaire sent successfully',
        resendSuccess: 'Questionnaire resent successfully',
        goToForms: 'Go to Forms',
        notFound: 'Questionnaire not found',
        notFoundDescription: 'The questionnaire you are looking for does not exist.',
        noFormsMessage: 'You have not created a form yet. Please navigate to',
        formsLink: 'Forms',
        noSubmission: 'No submission yet',
        notYetCompleted: 'This questionnaire has not been completed yet',
        schedule: {
          label: 'When to send',
          sendNow: 'Send now',
          anotherTime: 'Another time',
          date: 'Date',
          selectDate: 'Select date',
          explanation: {
            sendNow: 'This form will be sent immediately.',
            scheduled: 'This form will be sent on {date}.',
            pending: 'Please select a date to schedule this form.',
          },
          reminder: 'This questionnaire will be scheduled as:',
        },
      },
      photos: 'Photos',
      files: 'Files',
      habits: 'Habits',
      bookings: 'Bookings',
      trainingCalendar: 'Training',
      exerciseHistory: 'Exercise History',
      updates: 'Updates',
      appSettings: 'App Settings',
      clientNotFound: 'Client not found',
      viewAllClients: 'View all clients',
      allClients: 'All Clients',
      clientNotFoundDescription: 'We could not find a client with this id.',
      message: 'Message',
      messageAria: 'Open message with this client',
      resendInvite: 'Resend invite',
      resendInviteSuccess: 'Invite resent to {email}',
      resendInviteAria: 'Resend invite to this client',
      copyInvite: 'Copy invite',
      copyInviteAria: 'Copy invite link for this client',
      archive: 'Archive client',
      archiveAria: 'Archive client',
      archiveConfirmTitle: 'Are you sure you want to archive {firstName}?',
      archiveDescription: 'Archiving the user will remove their access to the app immediately. Archives can be restored which may incur charges depending on client limits.',
      yes: 'Yes',
      no: 'No',
      archivedSuccessfully: 'Client archived successfully',
      failedToArchive: 'Failed to archive client',
      athleteBio: 'Athlete bio',
      editBio: 'Edit bio',
      noBio: 'No bio available',
      bioPlaceholder: 'Enter athlete bio...',
      goals: 'Goals',
      editGoals: 'Edit goals',
      editGoal: 'Edit goal',
      noGoals: 'No goals available',
      goalsPlaceholder: 'Enter athlete goals...',
      goalNumber: 'Goal #{number}',
      goalTitle: 'Goal',
      goalPlaceholder: 'Enter goal...',
      targetDate: 'Target date',
      goalDate: 'Goal date',
      addGoal: 'Add goal',
      details: 'Details',
      detailsPlaceholder: 'Add additional notes...',
      injuries: 'Injuries',
      editInjuries: 'Edit injuries',
      editInjury: 'Edit injury',
      noInjuries: 'No injuries recorded',
      injuriesPlaceholder: 'Enter injury information...',
      injuryNumber: 'Injury #{number}',
      injuryTitle: 'Injury',
      injuryPlaceholder: 'Enter injury...',
      injuryDate: 'Injury date',
      addInjury: 'Add injury',
      athleteDetails: 'Athlete details',
      athleteName: 'Name',
      detailsComingSoon: 'Details coming soon',
      athleteTraining: 'Athlete training',
      trainingComingSoon: 'Training information coming soon',
      editDetails: 'Edit details',
      detailsUpdated: 'Details updated successfully',
      firstName: 'First name',
      firstNamePlaceholder: 'Enter first name',
      lastName: 'Last name',
      lastNamePlaceholder: 'Enter last name',
      fullName: 'Full name',
      email: 'Email',
      emailPlaceholder: 'Enter email address',
      age: 'Age',
      agePlaceholder: 'Enter age',
      birthDate: 'Birth Date',
      selectBirthDate: 'Select birth date',
      weight: 'Weight',
      weightPlaceholder: 'Enter weight (kg)',
      height: 'Height',
      heightPlaceholder: 'Enter height (cm)',
      category: 'Category',
      selectCategory: 'Select category',
      online: 'Online',
      inPerson: 'In-person',
      hybrid: 'Hybrid',
      gender: 'Gender',
      selectGender: 'Select gender',
      male: 'Male',
      female: 'Female',
      preferNotToSay: 'Prefer not to say',
      phone: 'Phone',
      phonePlaceholder: 'Enter phone number',
      country: 'Country',
      countryPlaceholder: 'Enter country',
      clientFor: 'Client for',
      currentAssignedItem: 'Current Assigned Item',
      noAssignedItem: 'No assigned item',
      viewAssignedItem: 'View {name}',
      weightStrengthOverview: 'Weight & Strength Overview',
      strengthOverview: 'Strength Overview',
      weightOverview: 'Weight Overview',
      noChartData: 'No chart data available',
      settings: {
        title: 'Settings',
        danger: {
          title: 'Danger',
          description: 'Manage dangerous actions for this athlete',
          cardTitle: 'Danger Zone',
          cardDescription: 'These actions cannot be undone. Please be careful.',
          archiveTitle: 'Archive client',
          archiveDescription: 'Archiving the user will remove their access to the app immediately. Archives can be restored.',
          archiveButton: 'Archive',
          deleteTitle: 'Delete client',
          deleteDescription: 'Deleting the client will permanently remove all their data. This action cannot be undone.',
          deleteButton: 'Delete',
          deleteConfirmTitle: 'Are you sure you want to delete {firstName}?',
          deleteConfirmDescription: 'This action cannot be undone. All data associated with this client will be permanently deleted.',
          deleteConfirmButton: 'Delete',
          deleteSuccess: 'Client deleted successfully',
        },
      },
    },
    trainingCalendar: {
      today: 'Today',
      goToToday: 'Go to today',
      previousWeek: 'Previous week',
      nextWeek: 'Next week',
      undo: 'Undo',
      redo: 'Redo',
      weekView: '{count} week',
      weekViewPlural: '{count} weeks',
      oneWeek: '1 week',
      twoWeeks: '2 weeks',
      fourWeeks: '4 weeks',
      showOneWeek: 'Show 1 week at a time on screen',
      showTwoWeeks: 'Show 2 weeks at a time on screen',
      showFourWeeks: 'Show 4 weeks at a time on screen',
      twoWeeksRequirement: '2 weeks view requires total weeks to be divisible by 2',
      fourWeeksRequirement: '4 weeks view requires total weeks to be divisible by 4',
      addWorkoutOrProgram: 'Add workout or program',
      addWorkout: 'Add workout',
      addProgram: 'Add program',
      addWorkoutModal: 'Add workout',
      addWorkoutModalWithDate: 'Add workout - {date}',
      searchWorkouts: 'Search workouts',

      selectWorkout: 'Select workout {name}',
      type: 'Type',
      equipment: 'Equipment',
      addConfigurations: 'Add configurations',
      addOnlyForThisDay: 'Add only for this day',
      addOnlyForThisDayAria: 'Add only for this day',
      addThisWorkoutEvery: 'Add this workout every',
      days: 'days',
      numberOfDays: 'Number of days',
      repeatWeeklyOnDay: 'Repeat weekly on day',
      dayOfWeek: 'Day of week',
      selectRepeatEveryDays: 'Select repeat every days option',
      selectRepeatWeekly: 'Select repeat weekly option',
      add: 'Add',
      addAria: 'Add workout',
      cancel: 'Cancel',
      workoutDetails: 'Workout details',
      viewDetailsAria: 'View details for workout {name}',
      deleteWorkout: 'Delete workout',
      deleteWorkoutAria: 'Delete workout',
      exercise: 'exercise',
      exercises: 'exercises',
      notStarted: 'Not started',
      inProgress: 'In progress',
      completed: 'Completed',
      section: {
        regular: 'regular',
        amrap: 'amrap',
        timed: 'timed',
        regularDescription: 'Exercise for exercise. Follow the sets and reps specified.',
        amrapDescription: 'Track the total amount of rounds completed in the allocated time.',
        timedDescription: 'Track total duration until completion of assigned rounds.',
        timeSeconds: 'Time (s)',
        rounds: 'Rounds',
      },
      table: {
        name: 'Name',
        difficulty: 'Difficulty',
        length: 'Length',
        muscleGroup: 'Muscle Group',
        set: 'Set',
        type: 'Type',
        reps: 'Reps',
        weight: 'Weight',
        distance: 'Distance',
        duration: 'Duration',
        restSeconds: 'Rest (s)',
        status: 'Status',
        warmUp: 'Warm up',
        dropset: 'Dropset',
      },
      noExercises: 'No exercises',
      noSections: 'No sections available.',
      editWorkout: 'Edit workout',
      editWorkoutAria: 'Edit workout',
      close: 'Close',
      closeAria: 'Close workout details',
      addProgramModal: 'Add program',
      searchPrograms: 'Search programs',
      searchProgramsPlaceholder: 'Search programs',
      noProgramsFound: 'No programs found.',
      selectProgram: 'Select program {name}',
      startDate: 'Start date',

      selectStartDate: 'Select start date',
      endDate: 'End date',
      selectProgramAndStartDate: 'Select a program and start date',
      addProgramAria: 'Add program to calendar',
      cancelAddProgram: 'Cancel add program',
      saveAs: 'Save as',
      saveAsWorkout: {
        menuItem: 'Save as workout',
        title: 'Save as Workout',
        selectDate: 'Select date of workout you wish to use',
        selectDateAria: 'Select date for workout',
      },
      saveAsProgram: {
        menuItem: 'Save as program',
        title: 'Save as Program',
        fromDate: 'From date',
        toDate: 'To date',
        selectFromDateAria: 'Select start date for program',
        selectToDateAria: 'Select end date for program',
        selectDatePlaceholder: 'Select date',
        success: 'Program saved successfully',
        error: 'Failed to save program',
        errors: {
          fromDateRequired: 'From date is required',
          toDateRequired: 'To date is required',
          nameRequired: 'Program name is required',
          typeRequired: 'Program type is required',
          difficultyRequired: 'Difficulty is required',
          noWorkoutFound: 'No workouts found in the selected range',
        },
      },
      addExercise: 'Add exercise',
      addExerciseModal: 'Add Exercise',
      addExerciseModalWithDate: 'Add Exercise to {date}',
      searchExercisesPlaceholder: 'Search exercises...',
      searchExercises: 'Search exercises',
      noExercisesFound: 'No exercises found',

      /* Add Workout Side Panel */
      addWorkoutTitle: 'Add Workout',
      addWorkoutDescription: 'Select a workout from your library to add to this day.',
      configureWorkoutTitle: 'Configure Workout',
      configureWorkoutDescription: 'Configure how this workout should be added to the calendar.',
      addThisWorkoutWeeklyOn: 'Add this workout weekly on',
      configurations: 'Configurations',
      noExercisesInWorkout: 'No exercises in this workout',
      addExerciseToWorkout: 'Add exercise',
      createNewWorkout: 'Create new workout',
      or: 'OR',

      /* Add Program Side Panel */
      addProgramTitle: 'Add Program',
      addProgramDescription: 'Select a program from your library to add to the calendar.',
      configureProgramTitle: 'Configure Program',
      configureProgramDescription: 'Set the start date and other details for this program.',
      programStartDate: 'Start Date',
      programStartDateDescription: 'This program will be scheduled starting from this date.',

      /* Add Exercise Side Panel */
      addExerciseTitle: 'Add Exercise',
      addExerciseDescription: 'Select an exercise from your library to add as a single workout.',

      searchWorkoutsPlaceholder: 'Search workouts...',
      noWorkoutsFound: 'No workouts found. Try adjusting your filters.',

      /* Copy/Paste feature */
      holdShiftToPaste: 'Hold shift to paste to multiple days',
      saveToLibrary: 'Save to Library',
      alreadySaved: 'This workout is already saved to your library',

      /* Multi-select feature */
      multiSelect: {
        workoutsSelected: '{count} workout(s) selected',
        clickToAddWorkout: 'Click on a day to add this workout',
        clickToAddWorkouts: 'Click on a day to add this group of workouts',
        deleteAria: 'Delete selected workouts',
        copyAria: 'Copy selected workouts',
        selectWorkoutAria: 'Select workout {name}',
      },

    },
  },
  error: {
    notFound: {
      title: '404 Page',
      description: 'This is an example of a template for 404 error pages. Built with shadcn/ui, Tailwind CSS and Next.js.',
      heading: '404',
      message: 'Page Not Found',
      backToHome: 'Back to Home',
      backToHomeAria: 'Go back to home page',
    },
    rateLimit: {
      heading: 'Oops!',
      message: "We're sorry, an error occurred. Please try again in a few seconds.",
      tryAgain: 'Try Again',
    },
  },
  photos: {
    title: 'Photos',
    addPhoto: 'Add photo',
    compare: 'Compare',
    comparingProgressPhotos: 'Comparing progress photos',
    searchPlaceholder: 'Search photos...',
    emptyMessage: 'No photos found',
    emptyState: {
      title: 'No photos yet',
      subtitle: 'Start tracking your client\'s progress with photos',
    },
    columns: {
      fileName: 'File name',
      tags: 'Tags',
      uploadedAt: 'Uploaded at',
    },
    actions: {
      clearSelected: 'Clear selected',
      deleteSelected: 'Delete selected',
      moreOptions: 'More options for {fileName}',
    },
    filters: {
      tags: 'Tags',
      allTags: 'All tags',
    },
    form: {
      type: 'Type',
      front: 'Front',
      back: 'Back',
      side: 'Side',
      image: 'Image',
      dropImageHere: 'Drop your image here',
      orClickToSelect: 'or click to select',
      selectImage: 'Select image',
      changeImage: 'Change image',
      takenAt: 'Taken at',
      selectDate: 'Select date',
      notesPlaceholder: 'Add some notes about these progress photos...',
    },
    deleteDialog: {
      title: 'Delete photo',
      description: 'Are you sure you want to delete this photo? This action cannot be undone.',
    },
  },
  files: {
    title: 'Files',
    addFile: 'Add file',
    newFile: 'New file',
    yourLibrary: 'Your library',
    searchPlaceholder: 'Search files...',
    emptyMessage: 'No files found',
    noLibraryFiles: 'You have no files in your library.',
    libraryLink: 'Library',
    emptyState: {
      title: 'No files yet',
      subtitle: 'Get started by adding your first file',
    },
    columns: {
      pinned: 'Pinned',
      fileName: 'File name',
      type: 'Type',
      size: 'Size',
      tags: 'Tags',
    },
    actions: {
      pin: 'Pin file',
      unpin: 'Unpin file',
      clearSelected: 'Clear selected',
      deleteSelected: 'Delete selected',
      addToClients: 'Add to clients',
      addFilesToOneClient: 'Add files to 1 client',
      addFilesToClients: 'Add files to {count} clients',
      removeFile: 'Remove {fileName}',
      moreOptions: 'More options for {fileName}',
    },
    filters: {
      tags: 'Tags',
      allTags: 'All tags',
    },
    addToClients: {
      title: 'Add to clients',
      filesToBeAdded: 'Files to be added',
      noClientSelected: 'Please select at least one client',
      noFiles: 'No files selected',
    },
    editFile: {
      title: 'Edit file',
    },
    form: {
      fileName: 'File name',
      fileNamePlaceholder: 'Enter file name...',
      file: 'File',
      dropFileHere: 'Drop your file here',
      orClickToSelect: 'or click to select',
      selectFile: 'Select file',
      changeFile: 'Change file',
      tags: 'Tags',
      selectTags: 'Select tags...',
      tagsSelected: 'tags selected',
      searchTags: 'Search tags...',
      noTagsFound: 'No tags found',
      removeTag: 'Remove {tag}',
    },
    assignSuccessSingle: 'Successfully assigned {fileName} to {clientName}',
    assignSuccessFileMultiClient: 'Successfully assigned {fileName} to {count} clients',
    assignSuccessMultiFileSingleClient: 'Successfully assigned {count} files to {clientName}',
  },
  habits: {
    title: 'Habits',
    addHabit: 'Add habit',
    addHabitTitle: 'Add habit',
    athliLibrary: 'Athli library',
    yourLibrary: 'Your library',
    newHabit: 'New habit',
    searchPlaceholder: 'Search habits...',
    emptyMessage: 'No habits found',
    noLibraryHabits: 'You have no habits in your library.',
    libraryLink: 'Library',
    emptyState: {
      title: 'No habits found',
      subtitle: 'Get started by adding your first habit to your library.',
    },
    columns: {
      name: 'Name',
    },
    actions: {
      clearSelected: 'Clear selected',
      deleteSelected: 'Delete selected',
      duplicate: 'Duplicate',
      duplicateAria: 'Duplicate habit',
      addToClients: 'Assign to clients',
    },
    assignToClients: 'Assign to clients',
    assignToClientsTitle: 'Assign habits to clients',
    assignToOneClient: 'Assign to 1 client',
    assignToClientsCount: 'Assign to {count} clients',
    habitsToAssign: 'Habits to be assigned',
    noHabitsSelected: 'No habits selected',
    assignSuccessSingle: 'Successfully assigned {habitName} to {clientName}',
    assignSuccessHabitMultiClient: 'Successfully assigned {habitName} to {count} clients',
    assignSuccessMultiHabitSingleClient: 'Successfully assigned {count} habits to {clientName}',
    form: {
      name: 'Habit name',
      namePlaceholder: 'Enter habit name...',
      nameRequired: 'Habit name is required',
      nameMaxLength: 'Habit name must be 60 characters or less',
      description: 'Habit Description',
      descriptionPlaceholder: 'Enter habit description...',
      amountUnitPeriod: 'Amount, Unit & Period',
      amount: 'Amount',
      amountRequired: 'Amount is required',
      unit: 'Unit',
      unitRequired: 'Unit is required',
      period: 'Period',
      periodRequired: 'Period is required',
      daily: 'Daily',
      weekly: 'Weekly',
      textualRepresentation: 'Complete {amount} {unit} {period}',
      textualRepresentationDaily: 'every day',
      textualRepresentationWeekly: 'weekly',
      days: 'Days',
      daysRequired: 'At least one day must be selected',
      monday: 'Mon',
      tuesday: 'Tue',
      wednesday: 'Wed',
      thursday: 'Thu',
      friday: 'Fri',
      saturday: 'Sat',
      sunday: 'Sun',
      duration: 'Habit duration',
      durationLabel: 'days',
      durationRequired: 'Habit duration is required',
      reminder: 'Reminder',
      reminderTime: 'Time',
      reminderMessage: 'Message',
      reminderMessagePlaceholder: 'Enter reminder message...',
      units: {
        steps: 'steps',
        min: 'min',
        times: 'times',
        count: 'count',
        drink: 'drink',
        cups: 'cups',
        m: 'm',
        km: 'km',
        mile: 'mile',
        sec: 'sec',
        hour: 'hour',
        hours: 'hours',
        minutes: 'minutes',
        ml: 'ml',
        l: 'l',
        oz: 'oz',
        cal: 'cal',
        g: 'g',
        mg: 'mg',
        session: 'session',
        sessions: 'sessions',
        glasses: 'glasses',
      },
    },
    logHabit: 'Log habit',
    logHabitTitle: 'Log habit',
    editHabitTitle: 'Edit habit',
    selectHabitMessage: 'Select a habit to view',
    viewAll: 'All Habits',
    view: 'View',
    noLogsMessage: 'No logs found for this period.',
    timeFilter: {
      lastWeek: 'Last week',
      last2Weeks: 'Last 2 weeks',
      lastMonth: 'Last month',
      last3Months: 'Last 3 months',
      last6Months: 'Last 6 months',
      lastYear: 'Last year',
      allTime: 'All time',
    },
    logHabitForm: {
      habit: 'Habit',
      habitPlaceholder: 'Select a habit...',
      habitRequired: 'Habit is required',
      value: 'Value',
      valuePlaceholder: 'Enter value...',
      valueAria: 'Habit value',
      valueRequired: 'Value is required',
      valueInvalid: 'Value must be a positive number',
    },
    streak: {
      current: 'Current streak',
      best: 'Best streak',
      longest: 'Longest',
      days: 'days',
      weeks: 'weeks',
    },
  },
  forms: {
    title: 'Forms',
    create: {
      success: 'Successfully created {name}',
    },
    update: {
      success: 'Successfully updated form',
    },
    assign: {
      success: 'Successfully assigned form(s)',
    },
    reorder: {
      success: 'Successfully reordered questions',
    },
    newCheckIn: 'New Check-in',
    newQuestionnaire: 'New Questionnaire',
    checkInDetail: 'Check-in details',
    questionnaireDetail: 'Questionnaire details',
    notFound: 'Form not found',
    addForm: 'Add form',
    addFormTitle: 'Add form',
    addCheckIn: 'Add Check in',
    addQuestionnaire: 'Add Questionnaire',
    addCheckInTitle: 'Add Check in',
    addQuestionnaireTitle: 'Add Questionnaire',
    editFormTitle: 'Edit form',
    editDetailsAndSchedule: 'Edit details & schedule',
    newForm: 'New form',
    athliTemplates: 'Athli templates',
    template: 'Template',
    searchPlaceholder: 'Search forms...',
    emptyMessage: 'No forms found.',
    questions: 'questions',
    question: 'question',
    tabs: {
      checkIns: 'Check Ins',
      questionnaires: 'Questionnaires',
      submissions: 'Submissions',
      response: 'Response',
    },
    questionnaires: {
      title: 'Questionnaires',
      emptyState: {
        title: 'No questionnaires found',
        subtitle: 'Get started by creating your first questionnaire template.',
      },
      assignAlert: 'This questionnaire will be sent immediately to the selected clients.',
    },
    checkIns: {
      title: 'Check-ins',
      emptyState: {
        title: 'No check-ins found',
        subtitle: 'Get started by creating your first check-in template.',
      },
      review: {
        title: 'Review',
        pageTitle: 'Review Check-in',
        client: 'Client',
      },
    },
    assignToClients: 'Assign to clients',
    assignToClientsTitle: 'Assign forms to clients',
    assignToOneClient: 'Assign to 1 client',
    assignToClientsCount: 'Assign to {count} clients',
    formsToAssign: 'Forms to be assigned',
    noFormsSelected: 'No forms selected',
    clearSelected: 'Clear',
    actions: {
      duplicate: 'Duplicate',
      duplicateAria: 'Duplicate form',
    },
    searchAthletes: 'Search athletes...',
    noAthletesFound: 'No athletes found.',
    assignSuccessSingle: 'Successfully assigned {formName} to {clientName}',
    assignSuccessFormMultiClient: 'Successfully assigned {formName} to {count} clients',
    assignSuccessMultiFormSingleClient: 'Successfully assigned {count} forms to {clientName}',
    columns: {
      name: 'Name',
      description: 'Description',
      questionCount: '# Questions',
    },
    form: {
      name: 'Form name',
      namePlaceholder: 'Enter form name...',
      nameRequired: 'Form name is required',
      nameMaxLength: 'Form name must be less than 100 characters',
      description: 'Description',
      descriptionPlaceholder: 'Enter description...',
    },
    type: {
      label: 'Type',
      checkIn: 'Check in',
      questionnaire: 'Questionnaire',
      checkInInfo: 'Check-ins are recurring forms that can be scheduled to be sent to athletes on a regular basis',
      questionnaireInfo: 'Questionnaires are one-time forms that can be assigned and sent immediately to athletes at any time',
    },
    toast: {
      updateSuccess: 'Form updated successfully',
      updateError: 'Failed to update form',
      deleteSuccess: 'Form deleted successfully',
      deleteError: 'Failed to delete form',
    },
    detail: {
      notFound: 'Form not found',
      notFoundDescription: 'The form you are looking for does not exist.',
      breadcrumb: {
        forms: 'Forms',
      },
      tabs: {
        submissions: 'Submissions',
        response: 'Response',
      },
      columns: {
        question: 'Question',
        required: 'Required',
        type: 'Type',
        action: 'Action',
      },
      emptyQuestions: 'No questions yet.',
      actions: {
        reorder: 'Re-order',
        done: 'Done',
        addQuestion: 'Add question',
        duplicate: 'Duplicate',
        duplicateAria: 'Duplicate form',
      },
      addQuestion: {
        title: 'Add question',
        success: 'Question added successfully',
        question: 'Question',
        questionPlaceholder: 'Enter question...',
        required: 'Required',
        format: 'Format',
        formats: {
          text: 'Text',
          textSubtitle: 'Free text input',
          number: 'Number',
          numberSubtitle: 'Numeric input only',
          multipleChoice: 'Multiple Choice',
          multipleChoiceSubtitle: 'Select one or more options',
          scale: 'Scale',
          scaleSubtitle: '1-10',
          yesNo: 'Yes/No',
          yesNoSubtitle: 'Binary choice',
          images: 'Images',
          imagesSubtitle: 'Upload images',
          videos: 'Videos',
          videosSubtitle: 'Upload videos',
          date: 'Date',
          dateSubtitle: 'Select a date',
          rating: 'Rating',
          ratingSubtitle: '1-5 stars',
          signature: 'Signature',
          signatureSubtitle: 'Digital signature capture',
          progressPhoto: 'Progress photo',
          progressPhotoSubtitle: 'Automatically sync progress photos based on response',
          metrics: 'Metric',
          metricsSubtitle: 'Automatically sync metrics based on response',
        },
        syncsWith: 'SYNC',
        general: 'General',
        selectMetric: 'Select metric',
        selectMetricPlaceholder: 'Select a metric...',
        options: 'Options',
        optionNumber: 'Option {number}',
        optionPlaceholder: 'Enter option...',
        addOption: 'Add option',
        scaleQuestion: 'Scale question',
        from: 'From',
        to: 'To',
        numberOfImages: 'Number of images',
        numberOfVideos: 'Number of videos',
        pleaseAddMetric: 'Please add a metric',
        metricsDisabledNoClient: 'Assign this form to a client first, then edit via their profile',
        metricsDisabledNoMetrics: 'Add metrics to this client first',
      },
      editQuestion: {
        title: 'Edit question',
      },
      deleteQuestion: {
        success: 'Question deleted successfully',
      },
    },
  },
  nutrition: {
    title: 'Nutrition',
    tabs: {
      recipes: 'Recipes',
      recipeBooks: 'Recipe Books',
      ingredients: 'Ingredients',
    },
    recipes: {
      title: 'Recipes',
      comingSoon: 'Recipes coming soon',
    },
    recipeBooks: {
      title: 'Recipe Books',
      comingSoon: 'Recipe books coming soon',
    },
    ingredients: {
      title: 'Ingredients',
      comingSoon: 'Ingredients coming soon',
    },
  },
  flows: {
    title: 'Flows',
    addFlow: 'Add flow',
    editFlow: 'Edit flow',
    searchPlaceholder: 'Search flows...',
    emptyMessage: 'No flows found.',
    notFound: 'Flow not found',
    notFoundDescription: 'The flow you are looking for does not exist.',
    actions: {
      clearSelected: 'Clear selected',
      duplicate: 'Duplicate',
      duplicateAria: 'Duplicate flow',
    },
    columns: {
      name: 'Name',
      description: 'Description',
      stepCount: 'Steps',
    },
    form: {
      name: 'Flow name',
      namePlaceholder: 'Enter flow name...',
      nameRequired: 'Flow name is required',
      nameMaxLength: 'Flow name must be less than 100 characters',
      description: 'Description',
      descriptionPlaceholder: 'Enter description...',
    },
    templateNotice: 'Template flows with placeholders require editing after creation. Edit the action nodes to select your specific content.',
    useTemplate: 'Use this template',
    editor: {
      deleteCheckTitle: 'Delete check',
      deleteCheckDescription: 'Deleting this check node will also remove all actions and conditions within its branches. This action cannot be undone.',
    },
  },
  onboarding: {
    title: 'Onboarding',
    addOnboarding: 'Add onboarding',
    editOnboarding: 'Edit onboarding',
    searchPlaceholder: 'Search onboarding flows...',
    emptyMessage: 'No onboarding flows found.',
    notFound: 'Onboarding not found',
    notFoundDescription: 'The onboarding flow you are looking for does not exist.',
    columns: {
      name: 'Name',
      description: 'Description',
      stepCount: 'Steps',
      status: 'Status',
      active: 'Active',
    },
    status: {
      published: 'Published',
      draft: 'Draft',
    },
    publish: 'Publish',
    unpublish: 'Unpublish',
    publishing: 'Publishing...',
    unpublishing: 'Unpublishing...',
    activated: '{name} is now active',
    deactivated: '{name} has been deactivated',
    form: {
      name: 'Onboarding name',
      namePlaceholder: 'Enter onboarding name...',
      nameRequired: 'Onboarding name is required',
      nameMaxLength: 'Onboarding name must be less than 100 characters',
      description: 'Description',
      descriptionPlaceholder: 'Enter description...',
    },
    emptyState: {
      title: 'No onboarding flows yet',
      subtitle: 'Create an onboarding flow to automatically welcome new clients',
    },
  },
  metrics: {
    title: 'Metrics',
    addMetric: 'Add metric',
    addMetricTitle: 'Add metric',
    editMetricTitle: 'Edit metric',
    logMetric: 'Log metric',
    logMetricTitle: 'Log metric',
    logMetricForm: {
      metric: 'Metric',
      metricPlaceholder: 'Select a metric...',
      metricRequired: 'Metric is required',
      value: 'Value',
      valuePlaceholder: 'Enter value...',
      valueAria: 'Metric value',
      valueRequired: 'Value is required',
      valueInvalid: 'Value must be a positive number',
    },
    timeFilter: {
      lastWeek: 'Last week',
      last2Weeks: 'Last 2 weeks',
      lastMonth: 'Last month',
      last3Months: 'Last 3 months',
      last6Months: 'Last 6 months',
      lastYear: 'Last year',
      allTime: 'All time',
    },
    searchPlaceholder: 'Search metrics...',
    searchAria: 'Search metrics',
    emptyMessage: 'No metrics found.',
    emptyState: {
      title: 'No metrics found',
      subtitle: 'Get started by adding your first metric to your library.',
    },
    noLogsMessage: 'No logs found for this period.',
    viewAll: 'All Metrics',
    view: 'View',
    selectMetricMessage: 'Select a metric to view',
    tabs: {
      newMetric: 'New metric',
      athliLibrary: 'Athli library',
      yourLibrary: 'Your library',
    },
    noLibraryMetrics: 'You don\'t have any metrics in your library yet. Create metrics in',
    libraryLink: 'Library',
    columns: {
      name: 'Name',
      unit: 'Unit',
      description: 'Description',
    },
    form: {
      name: 'Name',
      namePlaceholder: 'Enter metric name...',
      nameAria: 'Metric name',
      nameRequired: 'Metric name is required',
      unit: 'Unit',
      unitPlaceholder: 'Enter unit (e.g., kg, cm, %)...',
      unitAria: 'Metric unit',
      description: 'Description',
      descriptionPlaceholder: 'Enter description...',
      descriptionAria: 'Metric description',
    },
    noMetricsFound: 'No metrics found matching "{query}"',
    actions: {
      clearSelected: 'Clear selected',
      duplicate: 'Duplicate',
      duplicateAria: 'Duplicate metric',
      assignToClients: 'Assign to clients',
      deleteAria: 'Delete metric {name}',
    },
    assignToClientsTitle: 'Assign metrics to clients',
    assignToOneClient: 'Assign to 1 client',
    assignToClientsCount: 'Assign to {count} clients',
    metricsToAssign: 'Metrics to be assigned',
    noMetricsSelected: 'No metrics selected',
    assignSuccessSingle: 'Successfully assigned {metricName} to {clientName}',
    assignSuccessMetricMultiClient: 'Successfully assigned {metricName} to {count} clients',
    assignSuccessMultiMetricSingleClient: 'Successfully assigned {count} metrics to {clientName}',
    searchAthletes: 'Search athletes...',
    noAthletesFound: 'No athletes found.',
    schedule: {
      addLogFrequency: 'Add log frequency',
      frequency: {
        label: 'Log frequency',
        daily: 'Daily',
        weekly: 'Weekly',
        biweekly: 'Biweekly',
        monthly: 'Monthly',
      },
      selectDays: 'Select days',
      selectDay: 'Select day',
      dayOfMonth: 'Day of month',
      monthly: {
        first: 'First',
        last: 'Last',
        specific: 'Specific',
        selectDay: 'Select day of month',
      },
      explanation: {
        daily: 'Log this metric on: {days}',
        weekly: 'Log this metric every week on {day}',
        biweekly: 'Log this metric every two weeks on {day}',
        monthlyFirst: 'Log this metric on the first of each month',
        monthlyLast: 'Log this metric on the last day of each month',
        monthlySpecific: 'Log this metric on the {day} of each month',
      },
    },
  },
  referAndEarn: {
    hero: {
      title: 'Share Athli, get $20',
      description: 'Invite your coaching friends to Athli. When they subscribe, you both get $20 credit towards your next invoice. No limits on referrals.',
    },
    emailPlaceholder: 'Enter your friend\'s email',
    invite: 'Invite',
    copyLink: 'Copy Link',
    copyCode: 'Copy Code',
    referralLinkCopied: 'Referral link copied!',
    referralCodeCopied: 'Referral code copied!',
    copyFailed: 'Failed to copy',
    enterEmail: 'Please enter an email address',
    invalidEmail: 'Please enter a valid email address',
    inviteSent: 'Invite sent successfully!',
    inviteFailed: 'Failed to send invite',
    viewReferrals: 'View Referrals',
    yourReferrals: 'Your Referrals',
    noReferrals: 'No referrals yet',
    noReferralsSubtitle: 'Share your referral link to start earning credits.',
    columns: {
      coach: 'Coach',
      date: 'Date',
      status: 'Status',
      credit: 'Credit',
      credits: 'Credits',
    },
    status: {
      trialStarted: 'Trial Started',
      trialEnded: 'Trial Ended',
      trialCancelled: 'Trial Cancelled',
      converted: 'Converted',
      creditReceived: 'Credit Received',
      accepted: 'Accepted Referral',
    },
    referrer: 'Referrer',
    credits: {
      totalEarned: 'Total Earned',
      active: 'Active Credits',
      activeDescription: 'Applied to your next invoice',
      used: 'Total Savings',
    },
    howItWorks: {
      title: 'How it works',
      step1: 'Referral signs up',
      step2: 'Referral pays',
      step3: 'You each get $20',
    },
  },
  checkInReviews: {
    title: 'Review Check-ins',
    emptyMessage: 'No check-ins awaiting review.',
    emptyState: {
      title: 'All caught up!',
      subtitle: 'No check-ins are currently awaiting your review. Great work!',
    },
  },
  notifications: {
    title: 'Notifications',
    showAll: 'Show all',
    showUnread: 'Show unread',
    markAllAsRead: 'Mark all as read',
    markAsRead: 'Mark as read',
    markAsUnread: 'Mark as unread',
    empty: 'No notifications',
    emptyUnread: 'No unread notifications',
  },
  trial: {
    daysLeft: {
      one: '1 day left',
      other: '{count} days left',
    },
    tooltip: 'Free trial period',
    viewPlanDetails: 'View plan details',
    upgrade: 'Upgrade',
    tooltipExpiringOne: 'Your free trial is expiring in 1 day',
    tooltipExpiring: 'Your free trial is expiring in {count} days',
    tooltipClickToView: 'Click to view your plan details',
  },
  access: {
    blocked: {
      title: 'Your Free Trial Has Ended',
      description: 'Your 30-day free trial has ended. Subscribe to continue using Athli and growing your coaching business.',
      subscribe: 'Subscribe Now',
      questions: 'Questions? Contact us at support@athli.io',
    },
  },
  business: {
    title: 'Business',
    stripe: {
      connectMessage: 'Connect your Stripe account',
      disconnectTitle: 'Disconnect Stripe?',
      disconnectDescription: 'Are you sure you want to disconnect your Stripe account? Your packages and coupons will no longer be synced.',
      disconnect: 'Disconnect',
    },
    tabs: {
      activity: 'Activity',
      packages: 'Packages',
      coupons: 'Coupons',
      sequences: 'Sequences',
    },
    activity: {
      grossRevenue: 'Gross Revenue',
      thisMonth: 'This Month',
      activeSubscriptions: 'Active Subscriptions',
      payingClients: 'Paying Clients',
      vsLastMonth: 'vs last month',
      paymentActivity: 'Payment Activity',
      columns: {
        date: 'Date',
        client: 'Client',
        athlete: 'Athlete',
        package: 'Package',
        amount: 'Amount',
        type: 'Type',
        status: 'Status',
        event: 'Event',
        description: 'Description',
      },
      type: {
        one_time: 'One-time',
        subscription: 'Recurring',
      },
      status: {
        succeeded: 'Succeeded',
        pending: 'Pending',
        failed: 'Failed',
        refunded: 'Refunded',
        disputed: 'Disputed',
        cancelled: 'Cancelled',
      },
      noPayments: 'No activity yet',
      noPaymentsSubtitle: 'Billing activity will appear here as clients subscribe, pay, and make changes',
      noStripe: 'Connect Stripe to start accepting payments',
      noStripeSubtitle: 'Set up your Stripe account to enable payment processing',
      allDates: 'All Dates',
      today: 'Today',
    },
    packages: {
      addPackage: 'Add Package',
      editPackage: 'Edit Package',
      noPackages: 'No packages yet',
      noPackagesSubtitle: 'Create a package to start selling your coaching services',
      noArchivedPackages: 'No archived packages',
      noArchivedPackagesSubtitle: 'Archived packages will appear here',
      columns: {
        name: 'Name',
        sales: 'Sales',
        refunds: 'Refunds',
        cancellations: 'Cancellations',
        revenue: 'Revenue',
        redemptions: 'Redemptions',
        description: 'Description',
        price: 'Price',
        type: 'Type',
        duration: 'Duration',
        active: 'Active',
        visible: 'Visible',
        archived: 'Archived',
        actions: 'Actions',
      },
      viewStats: 'View stats',
      redemptionsDialog: {
        title: 'Package Redemptions',
        couponsTitle: 'Coupon Usage',
        totalRedemptions: '{count} total redemptions',
        noCoupons: 'No coupons used for this package yet',
        couponColumn: 'Coupon',
        redemptionsColumn: 'Redemptions',
      },
      actions: {
        shareLink: 'Share link',
        archive: 'Archive',
        unarchive: 'Unarchive',
      },
      filter: {
        active: 'Active',
        archived: 'Archived',
      },
      form: {
        name: 'Package Name',
        namePlaceholder: 'e.g. Premium Coaching',
        description: 'Description',
        descriptionPlaceholder: 'Describe what this package includes...',
        features: 'Features',
        featurePlaceholder: 'e.g. Unlimited messaging',
        addFeature: 'Add feature',
        currency: 'Currency',
        billingType: 'Billing Type',
        oneTime: 'One-time',
        daily: 'Daily',
        weekly: 'Weekly',
        monthly: 'Monthly',
        yearly: 'Yearly',
        every3Months: 'Every 3 months',
        every6Months: 'Every 6 months',
        custom: 'Custom',
        freeTrial: 'Free Trial',
        freeTrialDays: 'Trial days',
        days: 'days',
        weeks: 'weeks',
        months: 'months',
        years: 'years',
        price: 'Price',
        pricePlaceholder: '0.00',
        stepDetails: 'Details',
        stepOnboarding: 'Onboarding',
        onboarding: 'Onboarding Flow',
        onboardingNone: 'None',
        onboardingDescription: 'When a client purchases this package, the selected onboarding flow will be triggered automatically.',
        sequence: 'Sequence',
        addSequence: 'Sequence',
        sequenceNone: 'None',
        sequenceDescription: 'When a client completes payment for this package, the selected sequence will be triggered automatically.',
        noSequences: 'No sequences created yet.',
        noSequencesLink: 'Create one',
        image: 'Package Image',
        imageUpload: 'Upload Image',
        imageRemove: 'Remove',
        imagePresets: 'Or choose a preset',
      },
      interval: {
        one_time: 'One-time',
        day: 'Daily',
        week: 'Weekly',
        month: 'Monthly',
        year: 'Yearly',
      },
      stripe: {
        notConnected: 'Connect Stripe to activate packages',
        connected: 'Stripe connected',
        connect: 'Connect Stripe',
        continueSetup: 'Continue Stripe Setup',
        dashboard: 'Stripe Dashboard',
      },
      stats: {
        purchases: 'Purchases',
        refunds: 'Refunds',
        revenue: 'Revenue',
      },
      toast: {
        created: 'Package created',
        updated: 'Package updated',
        deleted: 'Package deleted',
        activated: 'Package activated',
        deactivated: 'Package deactivated',
        stripeRequired: 'Connect Stripe before activating packages',
        linkCopied: 'Checkout link copied to clipboard',
      },
    },
    coupons: {
      addCoupon: 'Add Coupon',
      editCoupon: 'Edit Coupon',
      noCoupons: 'No coupons yet',
      noCouponsSubtitle: 'Create coupons for your clients',
      columns: {
        name: 'Name',
        code: 'Code',
        discount: 'Discount',
        duration: 'Duration',
        redemptions: 'Redemptions',
        expires: 'Expires',
        active: 'Active',
        status: 'Status',
        actions: 'Actions',
      },
      form: {
        name: 'Coupon Name',
        namePlaceholder: 'Name',
        code: 'Code',
        codePlaceholder: 'WINTER15',
        discountType: 'Discount Type',
        percentage: 'Percentage',
        fixedAmount: 'Fixed Amount',
        discountValue: 'Discount Value',
        valuePlaceholder: '0',
        currency: 'Currency',
        duration: 'Duration',
        oneTime: 'One-time',
        months: '{count} months',
        month: '{count} month',
        once: 'Once',
        forever: 'Forever',
        repeating: 'Repeating',
        onceHint: 'Discount applies to the first payment only',
        foreverHint: 'Discount applies to all payments forever',
        repeatingHint: 'Discount applies for the first {months} months',
        limitRedemptions: 'Limit Redemptions',
        maxRedemptions: 'Max redemptions',
        expirationDate: 'Expiration Date',
        selectDate: 'Select a date',
        unlimited: 'unlimited',
        never: 'Never',
      },
      toast: {
        created: 'Coupon created',
        updated: 'Coupon updated',
        deleted: 'Coupon deleted',
        duplicateCode: 'A coupon with this code already exists',
      },
    },
    sequences: {
      addSequence: 'Add sequence',
      editSequence: 'Edit sequence',
      searchPlaceholder: 'Search sequences...',
      emptyMessage: 'No sequences found.',
      notFound: 'Sequence not found',
      notFoundDescription: 'The sequence you are looking for does not exist.',
      columns: {
        name: 'Name',
        description: 'Description',
        stepCount: 'Steps',
      },
      form: {
        name: 'Sequence name',
        namePlaceholder: 'Enter sequence name...',
        nameRequired: 'Sequence name is required',
        nameMaxLength: 'Sequence name must be less than 100 characters',
        description: 'Description',
        descriptionPlaceholder: 'Enter description...',
      },
      emptyState: {
        title: 'No sequences yet',
        subtitle: 'Create a sequence to automate actions after payment',
      },
      toast: {
        created: 'Sequence created',
        updated: 'Sequence updated',
        deleted: 'Sequence deleted',
        deleteLinked: 'Cannot delete sequence: it is linked to one or more packages',
      },
    },
  },
  pricing: {
    sectionTitle: 'Pricing that Scales with You',
    sectionSubtitle: 'Start free, upgrade as you grow. Simple plans built for coaches at every stage.',
    getStarted: 'Get Started',
    popular: 'Most Popular',
    monthly: 'Monthly',
    annual: 'Annual',
    twoMonthsFree: 'Save 17% with annual billing',
    month: 'mo',
    year: 'yr',
    plan: 'plan',
    upTo: 'Up to',
    startingAt: 'Starting at',
    clients: 'clients',
    extraClient: 'extra client',
    addon: 'ADD-ON',
    addToMyPlan: 'Add to my plan',
    showFeatures: 'Show all features',
    hideFeatures: 'Hide features',
    addonsSection: 'Supercharge with Add-ons',
    chooseClients: {
      title: 'I want to coach',
      subtitle: 'On {plan}, {clients} clients',
      growBusiness: 'Grow your business up to {limit} clients',
    },
    starter: {
      name: 'Starter',
      priceShort: '$0/mo',
      description: 'Perfect for getting started with up to 5 clients',
      features: [
        'Coach Mobile App (iOS & Android)',
        'Client Mobile App (iOS & Android)',
        'Workouts & Programs',
        'Training Calendar',
        'Exercise Library (1,700+)',
        'Client Messaging',
      ],
      newFeatures: [],
    },
    pro: {
      name: 'Pro',
      priceShort: 'From $20/mo',
      description: 'Everything you need to grow your coaching business',
      features: [
        'Coach Mobile App (iOS & Android)',
        'Client Mobile App (iOS & Android)',
        'Workouts & Programs',
        'Training Calendar',
        'Exercise Library (1,700+)',
        'Client Messaging',
      ],
      newFeatures: [
        'AI Workout Builder',
        'Custom Exercises',
        'Custom Workout Sections',
        'Questionnaires',
        'Check-ins',
        'Habits & Metrics Tracking',
        '5GB On Demand Files',
      ],
    },
    max: {
      name: 'Max',
      priceShort: 'From $99/mo',
      description: 'For established coaches managing larger client bases',
      features: [
        'Coach Mobile App (iOS & Android)',
        'Client Mobile App (iOS & Android)',
        'Workouts & Programs',
        'Training Calendar',
        'Exercise Library (1,700+)',
        'Client Messaging',
      ],
      newFeatures: [
        'Unlimited On Demand Files',
        'Broadcast Messaging',
        'AI Todo List',
        'Priority Support',
        'Lower extra client pricing',
      ],
    },
    extraClients: {
      title: 'Need More Clients?',
      description: 'Add extra clients beyond your plan limit at a fixed rate per client.',
      additional: 'Extra clients',
      total: 'Total capacity',
    },
    addons: {
      title: 'Add-ons',
      description: 'Supercharge your coaching with powerful add-ons.',
      automations: {
        name: 'Automations',
        description: 'Onboarding sequences and automated flows',
        features: [
          'Automated onboarding sequences',
          'Custom trigger-based flows',
          'Scheduled actions and reminders',
          'Multi-step automation chains',
        ],
      },
      broadcast: {
        name: 'Broadcast',
        description: 'Send messages to multiple clients at once',
        features: [
          'Message multiple clients at once',
          'Quick bulk communication',
        ],
      },
      aiAssistant: {
        name: 'AI Assistant (Lyra)',
        description: 'Your all-round coaching assistant',
        features: [
          'Generate workouts and programs',
          'Client progress tracking and analysis',
          'Ask anything about your clients',
          'Find clients who need attention',
        ],
      },
      payments: {
        name: 'Payments',
        description: 'Collect payments from your clients via Stripe',
        features: [
          'Stripe integration',
          'Packages and subscription management',
          'Automated payment collection',
          'Revenue analytics dashboard',
          'Coupons and discounts',
        ],
      },
    },
    summary: {
      title: 'Plan Summary',
      plan: 'Plan',
      extraClients: 'Extra clients',
      addons: 'Add-ons',
      total: 'Total',
      billedAnnually: 'Billed Annually',
      capacity: 'Total client capacity',
      cta: 'Start a free trial',
      pricesInUSD: 'All prices are in USD',
      termsPrefix: 'By continuing, you agree to our',
      termsLink: 'Terms of Service',
      and: 'and',
      privacyLink: 'Privacy Policy',
    },
  },
  pricingComparison: {
    title: 'Compare Plans in Detail',
    subtitle: 'See exactly what you get at every tier.',
    featuresHeader: 'Features',
    clientsIncluded: 'Clients Included',
    sections: [
      {
        title: 'Mobile Apps',
        features: ['Coach Mobile App (iOS & Android)', 'Client Mobile App (iOS & Android)'],
      },
      {
        title: 'Core Features',
        features: ['Workouts & Programs', 'Training Calendar', 'Exercise Library (1,700+)', 'Client Messaging'],
      },
      {
        title: 'Advanced Features',
        features: ['AI Workout Builder', 'Custom Exercises', 'Custom Workout Sections'],
      },
      {
        title: 'Client Management',
        features: ['Questionnaires', 'Check-ins', 'Habits Tracking', 'Metrics Tracking', 'Photo Tracking', 'Exercise History'],
      },
      {
        title: 'Storage & Support',
        features: ['File Storage', 'AI Todo List', 'Broadcast Messaging', 'Priority Support'],
      },
    ],
  },
  assistant: {
    chats: 'Chats',
    chatHistory: 'Chat history',
    newChat: 'New chat',
    searchChats: 'Search chats...',
    voiceInput: 'Voice input',
    stopListening: 'Stop listening',
    capabilities: {
      training: {
        createWorkouts: 'Create workouts & sections',
        searchExercises: 'Search exercises',
        assignToClients: 'Assign to clients',
      },
      clients: {
        viewSearch: 'View & search clients',
        profilesGoals: 'Profiles, goals & injuries',
        findInactive: 'Find inactive clients',
      },
      analytics: {
        analyzeProgress: 'Analyze progress',
        completionRates: 'Completion rates',
        metricsCheckins: 'Metrics & check-ins',
      },
      library: {
        browseWorkouts: 'Browse workouts',
        checkinTemplates: 'Check-in templates',
        trackedMetrics: 'Tracked metrics',
      },
    },
  },
  upgrade: {
    toPro: 'Upgrade to Pro',
    toMax: 'Upgrade to Max',
  },
  common: {
    settings: 'Settings',
    compare: 'Compare',
    download: 'Download',
    changePicture: 'Change Picture',
    timezone: 'Timezone',
    selectExerciseProgress: 'Select an exercise to view progress',
    assignHabit: 'Assign Habit',
    logHabit: 'Log a Habit',
    enterLog: 'Enter a Log',
    assignMetric: 'Assign Metric',
    logMetric: 'Log a Metric',
    athleteWorkouts: 'Athlete Workouts',
    addPhoto: 'Add photo',
    selectDateViewPhotos: 'Select a date to view photos',
    selectPhotoView: 'Select a photo to view',
    restoreClient: 'Restore Client',
    clientDetails: 'Client Details',
    changeProgram: 'Change program',
    messageClient: 'Message Client',
    closeHistoryPanel: 'Close history panel',
    closeChartView: 'Close chart view',
    chartView: 'Chart view',
    history: 'History',
    workoutPreview: 'Workout Preview',
    viewProfile: 'View profile',
    message: 'Message',
    sendMessageTo: 'Send a message to {name}',
    online: 'Online',
    hybrid: 'Hybrid',
    inProgress: 'In Progress',
    requestNewFlow: 'Request a new flow',
    newFeatureRequest: 'New Feature Request',
    addReply: 'Add Reply',
    selectFeatureRequest: 'Select a feature request to view details',
    deleteFeatureRequest: 'Delete Feature Request?',
    deleteReply: 'Delete Reply?',
    previewPackages: 'Preview Packages',
    deactivate: 'Deactivate',
    paymentsAddonRequired: 'Payments Add-on Required',
    moveToFolder: 'Move to folder',
    createFolder: 'Create Folder',
    editFolder: 'Edit Folder',
    assignFolder: 'Assign Folder',
    move: 'Move',
    duplicate: 'Duplicate',
    duplicateSelected: 'Duplicate selected',
    insertWeek: 'Insert week',
    editDetails: 'Edit details',
    saveToLibrary: 'Save to Library',
    alreadySaved: 'This workout is already saved',
    dropExerciseHere: 'Drop your exercise here',
    dropSectionHere: 'Drop section here',
    athliAI: 'Athli AI',
    manual: 'Manual',
    addExercise: 'Add exercise',
    createSection: 'Create section',
    currentPlan: 'Current Plan',
    activeClients: 'Active Clients',
    freeTrialDialog: "You're on a Free Trial",
    settingsNavigation: 'Settings navigation',
    libraryNavigation: 'Library navigation',
    closeInboxPanel: 'Close inbox panel',
    openInboxPanel: 'Open inbox panel',
    sleep: 'Sleep',
    mood: 'Mood',
    energy: 'Energy',
    stress: 'Stress',
    soreness: 'Soreness',
    searchPlaceholder: 'Search...',
    searchTimezone: 'Search timezone...',
    selectCheckin: 'Select a check-in',
    pullingUpGoodStuff: 'Pulling up the good stuff...',
    noHabitsAssigned: 'No habits assigned',
    noExerciseHistory: 'No exercise history',
    completeWorkoutsToSeeHistory: 'Complete workouts to see exercise progress and history here.',
    noFlowsFound: 'No flows found',
    flowsMissing: 'The default system flows seem to be missing.',
    publishFlows: 'Publish automation flows to automatically respond to client actions like missed check-ins or completed workouts.',
    activateOnboarding: 'Activate onboarding flows to automatically guide new clients through your setup process.',
    automatedTasks: 'Automated tasks and reminders will appear here when there\'s something that needs your attention',
    addOrganizeTasks: 'Add and organize tasks to keep track of your coaching work and client management',
    noCheckinAssigned: 'No check-ins assigned',
    searchExercises: 'Search exercises...',
    searchRequests: 'Search requests...',
    enterTitle: 'Enter a short, descriptive title',
    provideDetails: 'Provide more details about your feature request (optional)',
    writeReply: 'Write your reply...',
    unarchiveConfirmation: 'Are you sure you want to unarchive this client? They will regain access to the app immediately.',
    selectImageFile: 'Please select an image file',
    imageSizeLimit: 'Image size must be less than 5MB',
    noMetricsAssigned: 'No metrics assigned',
    noQuestionnairesAssigned: 'No questionnaires assigned',
    startWeek: 'Start Week',
    endWeek: 'End Week',
    enterFullName: 'Enter your full name',
    editMessage: 'Edit your message...',
    createCheckinSubtitle: "Create check-in forms to regularly collect feedback and monitor your clients' progress",
    assignCheckinDescription: 'Assign check-in forms to track client progress with regular updates and insights.',
    createQuestionnaireSubtitle: 'Build questionnaires to gather detailed information from your clients about their goals and preferences',
    assignQuestionnaireDescription: 'Assign questionnaires to gather detailed information from your clients about their goals and preferences.',
    uploadFilesSubtitle: 'Upload and organize files to share with your clients - training plans, nutrition guides, and resources',
    askMeAnything: 'Ask me anything...',
    createHabitsSubtitle: 'Create daily habits to help your clients build consistency and track their progress over time',
    searchHabits: 'Search habits...',
    defineMetricsSubtitle: 'Define custom metrics to track and measure your clients\' performance and progress',
    addMetricsToFolder: 'Add metrics to this folder to organize your library',
    workoutTitle: 'Workout title',
  },
  toasts: {
    onboardingCreated: 'Onboarding created successfully',
    failedUpdateFlow: 'Failed to update flow status',
    failedLoadRequests: 'Failed to load feature requests',
    failedLoadDetails: 'Failed to load request details',
    failedToggleUpvote: 'Failed to toggle upvote',
    featureDeleted: 'Feature request deleted',
    replyDeleted: 'Reply deleted',
    mustBeLoggedIn: 'You must be logged in to create a feature request',
    mustBeLoggedInReply: 'You must be logged in to reply',
    enterTitle: 'Please enter a title',
    enterMessage: 'Please enter a message',
    featureCreated: 'Feature request created!',
    replyAdded: 'Reply added!',
    unableGenerateLink: 'Unable to generate referral link. Please try again.',
    unableGenerateCode: 'Unable to generate referral code. Please try again.',
    failedLoadWorkouts: 'Failed to load workouts',
    failedLoadWorkoutDetails: 'Failed to load workout details',
    failedUnarchive: 'Failed to unarchive client(s)',
    unableGenerateInvite: 'Unable to generate invite link. Please try again.',
    failedUnarchiveClient: 'Failed to unarchive client',
    failedResendInvitation: 'Failed to resend invitation. Please try again.',
    failedAssignFiles: 'Failed to assign files',
    logUpdated: 'Log updated successfully',
    failedUpdateLog: 'Failed to update log',
    logDeleted: 'Log deleted successfully',
    failedDeleteLog: 'Failed to delete log',
    errorDuringGeneration: 'An error occurred during generation',
    clientProfileNotSupported: 'Client profile updates are not yet supported via AI assistant',
    copiedToClipboard: 'Copied to clipboard',
    exercisesDuplicated: 'Exercises duplicated successfully',
    failedAssignQuestionnaires: 'Failed to assign questionnaires',
    failedCreateWorkout: 'Failed to create workout',
    failedDeleteExercise: 'Failed to delete exercise',
    failedDeleteExercises: 'Failed to delete exercises',
    failedDeleteHabit: 'Failed to delete habit',
    failedDeleteMetric: 'Failed to delete metric',
    failedDeleteProgram: 'Failed to delete program',
    failedDeletePrograms: 'Failed to delete programs',
    failedDeleteWorkout: 'Failed to delete workout',
    failedDeleteWorkouts: 'Failed to delete workouts',
    failedDisconnectStripe: 'Failed to disconnect Stripe account',
    failedDuplicateExercises: 'Failed to duplicate exercises',
    failedDuplicateWorkout: 'Failed to duplicate workout',
    failedGenerateWorkout: 'Failed to generate workout',
    failedLoadFullWorkoutDetails: 'Failed to load full workout details',
    failedLogHabit: 'Failed to log habit',
    failedLogMetric: 'Failed to log metric',
    failedOpenStripeDashboard: 'Failed to open Stripe Dashboard',
    failedSaveChanges: 'Failed to save changes',
    failedSaveHabit: 'Failed to save habit',
    failedSaveMetric: 'Failed to save metric',
    failedSaveSectionToLibrary: 'Failed to save section to library',
    failedSaveWorkout: 'Failed to save workout',
    failedSaveWorkouts: 'Failed to save workouts',
    failedSendMessage: 'Failed to send message',
    failedStartStripeConnection: 'Failed to start Stripe connection',
    failedUpdateCoupon: 'Failed to update coupon',
    failedUpdateCoupons: 'Failed to update coupons',
    failedUpdateDetails: 'Failed to update details',
    failedUpdateHabit: 'Failed to update habit',
    failedUpdateMetric: 'Failed to update metric',
    failedUpdatePackage: 'Failed to update package',
    failedUpdatePackages: 'Failed to update packages',
    failedUpdateWorkout: 'Failed to update workout',
    fileAdded: 'File added successfully',
    habitDeleted: 'Habit deleted successfully',
    habitLogged: 'Habit logged successfully',
    habitSaved: 'Habit saved successfully',
    habitUpdated: 'Habit updated successfully',
    maxFilesPerMessage: 'Maximum 4 files per message',
    metricAdded: 'Metric added successfully',
    metricDeleted: 'Metric deleted successfully',
    metricLogged: 'Metric logged successfully',
    metricSaved: 'Metric saved successfully',
    metricUpdated: 'Metric updated successfully',
    nameRequired: 'Name is required',
    completeStripeOnboarding: 'Please complete Stripe onboarding',
    confirmNewEmail: 'Please confirm your new email address via the link sent to your inbox',
    enterWorkoutName: 'Please enter a workout name',
    fillAllFields: 'Please fill out all fields',
    selectExerciseForCards: 'Please select an exercise for all exercise cards',
    profilePictureUpdated: 'Profile picture updated successfully',
    profileUpdated: 'Profile updated successfully',
    sectionCreationComingSoon: 'Section creation coming soon',
    sectionSavedToLibrary: 'Section saved to library',
    stripeConnected: 'Stripe account connected successfully',
    stripeDisconnected: 'Stripe account disconnected',
    programAssigned: 'Successfully assigned program',
    exerciseDeleted: 'Successfully deleted exercise',
    programDeleted: 'Successfully deleted program',
    actionNotSupported: 'This action type is not yet supported',
    folderEmpty: 'This folder is empty',
    timezoneUpdated: 'Timezone updated successfully',
    workoutAddedToLibrary: 'Workout added to your library!',
    workoutCreated: 'Workout created successfully',
    workoutDetailsMissing: 'Workout details are missing',
    workoutUpdated: 'Workout updated',
    workoutUpdatedSuccessfully: 'Workout updated successfully',
    validationAmrapDuration: 'AMRAP duration is required',
    validationCircuitsRounds: 'Number of rounds is required',
    validationAuxiliaryCategory: 'Category is required',
    validationMissingConfig: 'Section configuration is incomplete',
    validationNoExercises: 'No exercises added',
    validationMissing: 'missing',
    validationMoreIssues: 'more issues',
    fieldReps: 'reps',
    fieldWeight: 'weight',
    fieldDistance: 'distance',
    fieldDuration: 'duration',
    fieldTempo: 'tempo',
    unnamedExercise: 'Unnamed exercise',
  },
} as const;

export type EnMessages = typeof enMessages;
