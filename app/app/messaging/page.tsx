...
const MessagingPage = () => {
  ...
  // Read contact ID from URL hash on mount and when hash changes
  React.useEffect(() => {
    if (typeof window === 'undefined') return

    const updateSelectedContactFromLocation = () => {
      const hash = window.location.hash.slice(1) // Remove the #
      let candidateContactId: string | null = null

      const hashMatch = hash.match(/^\/app\/messaging\/(.+)$/)

      if (hashMatch && hashMatch[1]) {
        candidateContactId = hashMatch[1]
      }
    ...
    // Check on mount
    updateSelectedContactFromLocation()
  ...
  // Update URL hash when contact is selected
  React.useEffect(() => {
    if (typeof window === 'undefined') return

    if (selectedContactId) {
      const newHash = `#/app/messaging/${selectedContactId}`
      if (window.location.hash !== newHash) {
        window.history.replaceState(null, '', `/${newHash}`)
      }
    } else {
      // If no contact selected, just go to /app/messaging
      const newHash = '#/app/messaging'
      if (window.location.hash !== newHash) {
        window.history.replaceState(null, '', `/${newHash}`)
      }
    }
